...
public class Entity : MonoBehaviour {
    ...
    public GameObject focoEffectSuccess { private set; get; }
    ...
    public void OnAttackEnd() {
        ...
        if (this.SyncEntity == null || this.SyncEntity.FSM == null ||
            this.SyncEntity.FSM.State != BlGame.FSM.FsmState.FSM_STATE_RELEASE) {
            return;
        }

        int id =
            UnityEngine.Random.Range(0, ConfigReader.HeroXmlInfoDict[SyncEntity.NpcGUIDType].n32RandomAttack.Count);
    ...
    public void CrossFadeSqu(string name) { GetComponent<Animation>().CrossFadeQueued(name); }
    ...
    private void OnTriggerEnter(Collider other) {
        ...
        if (entity.SyncEntity.EntityCamp != player.EntityCamp ||
            entity.SyncEntity.entityType != EntityType.Building  // 如果进入的不是友方祭坛、
    ...
    private void LoadFuryEffect() {
        if (null == furyEffect) {
            // furyEffect = GameObject.Instantiate(Resources.Load(GameDefine.GameConstDefine.FuryGasExplosionEffect)) as
            // GameObject;
            ResourceUnit furyEffectUnit = ResourcesManager.Instance.loadImmediate(
                GameDefine.GameConstDefine.FuryGasExplosionEffect, ResourceType.PREFAB);
    ...
    public void ShowTrail() { Invoke("ShowTrailDelay", 1.0f); }
