...
public class SocialWindow : BaseWindow {
    ...
    private void CanelFriendToBlack(GameObject go) { mSureToBlack.transform.parent.gameObject.SetActive(false); }
    ...
    private void CanelDelFriend(GameObject go) { mSureDel.transform.parent.gameObject.SetActive(false); }
    ...
    private void OpenAddFriend(GameObject go) { mSendAddFriend.transform.parent.gameObject.SetActive(true); }
    ...
    private void SendAddFriendBlack(GameObject go) {  // 通过ID查找用户
        if (mInputTemp.value != null && !string.IsNullOrEmpty(mInputTemp.value) &&
            (string.Compare(mInputTemp.value, PutName) != 0)) {
            if (ToggleID.value) {
                string pat = @"[\u4e00-\u9fa5]";
    ...
    private void EventError(EErrorCode arg1) { MsgInfoManager.Instance.ShowMsg((int)arg1); }
        ...
        foreach (var item in temp.Values) {
            ...
            if (item.isOnline == isOn) {
                ResourceUnit objUnit =
                    ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadFriendUI, ResourceType.PREFAB);
