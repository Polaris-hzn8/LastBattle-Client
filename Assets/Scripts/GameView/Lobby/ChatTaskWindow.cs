    ...
    void CreateMember(UInt64 temp) {
        ...
        if (FriendManager.Instance.AllFriends.TryGetValue(temp, out friend)) {
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadChatMember, ResourceType.PREFAB);
    ...
    void ViableTalk(string currStr) {
        ...
        if (FriendManager.Instance.AllFriends.TryGetValue(mLastTemp, out friend)) {
            if (!FriendManager.Instance.AllTalkDic.TryGetValue(mLastTemp, out cat)) {
                ...
                cat.SetMsgInfo(
                    mLastTemp, friend.NiceName, currStr, MsgTalk.ReadMsg, GameUserModel.Instance.GameUserHead, true);
    ...
    public void ShowTalkInfo(UInt64 sGUID) {
        ...
        if (mLastTemp == sGUID) {
            ...
            if (FriendManager.Instance.AllTalkDic.TryGetValue(sGUID, out cat)) {
                ...
                ShowTalk(sGUID,
                         cat.NickName,
                         cat.MsgInfo[cat.MsgInfo.Count - 1].msg,
                         cat.MsgInfo[cat.MsgInfo.Count - 1].head,
                         cat.MsgInfo[cat.MsgInfo.Count - 1].isLocalPlayer);
            }
            ChatTaskCtrl.Instance.SetNewChat();
        } else {
            IsVisable = true;
