...
public class UIStartSelfDefine : MonoBehaviour {
    ...
    void OnEnable() {
        ...
        DoMoveIn();
        // UIGuide
        IGuideTaskManager.Instance().AddTaskStartListerner(EGameEvent.eGameEvent_UIGuideSelectPrimaryBtnStart,
                                                           StartIGuideTask);
        IGuideTaskManager.Instance().AddTaskStartListerner(EGameEvent.eGameEvent_UIGuideCreateRoomBtnStart,
                                                           StartIGuideTask);
    ...
    void MoveMethod(EventDelegate.Callback callback,
                    float xMove,
                    float durning,
                    UITweener.Method method = UITweener.Method.EaseIn,
    ...
    void OnDisable() {
        ...
        IGuideTaskManager.Instance().RemoveTaskStartListerner(EGameEvent.eGameEvent_UIGuideSelectPrimaryBtnStart,
                                                              StartIGuideTask);
        IGuideTaskManager.Instance().RemoveTaskStartListerner(EGameEvent.eGameEvent_UIGuideCreateRoomBtnStart,
                                                              StartIGuideTask);
    }
    void BtnCreateOrAdd() {
    ...
    public void CreateOrAddRoom() {
        if (string.Equals(BtnCreateRoom.GetComponentInChildren<UILabel>().text, CreateRoom)) {
            ...
            if (string.IsNullOrEmpty(InputRoomNum.label.text) || string.Equals(InputRoomNum.label.text, "-------")) {
                ...
                // if (msg != null)
                //     MsgInfoManager.Instance.ShowMsg((MsgInfoManager.MsgType)msg.msgType,
                //     (int)ERROR_TYPE.eT_NullRoomID);
            } else {
