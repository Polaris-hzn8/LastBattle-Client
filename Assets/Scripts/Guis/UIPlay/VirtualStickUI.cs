...
#region 私有方法
    ...
    void ShowStick() {
        ...
        transform.position = new Vector3(
            UICommonMethod.GetWorldPos(touchPos).x, UICommonMethod.GetWorldPos(touchPos).y, transform.position.z);
    ...
    void SendStop() { CGLCtrl_GameLogic.Instance.EmsgToss_AskStopMove(); }
    ...
    void SendMove() {
        ...
        Vector3 dealPos =
            entity.transform.position + dir * Time.deltaTime * PlayerManager.Instance.LocalPlayer.EntityFSMMoveSpeed;
    ...
    private void PlayerAdMove(Vector3 mvDir) {
        ...
        if (player.FSM.State == FsmState.FSM_STATE_DEAD || player.FSM.State == FsmState.FSM_STATE_RUN ||
            player.FSM.State == FsmState.FSM_STATE_FORCEMOVE || player.FSM.State == FsmState.FSM_STATE_RELIVE) {
            return;
        }
...
#region 变量区
    public static VirtualStickUI Instance { get; private set; }
    ...
    public StickState VirtualStickState { get; private set; }
