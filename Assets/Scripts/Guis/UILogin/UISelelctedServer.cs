...
public class UISelelctedServer : MonoBehaviour {
    ...
    void ShowAllServer(int type = 0) {  // type = 0  show name,type = 1 show addr
        for (int i = 0; i < nameList.Count; i++) {
            ...
            stateList.ElementAt(i).text =
                (type == 0) ? ("(" + SelectServerData.Instance.StateString[(int)(info.state)] + ")") : "";
    ...
    void CreateServerItems() {  // create items ,and label
        ...
        for (int i = 0; i < count * 6; i++) {
            ResourceUnit sceneRootUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadServerItems, ResourceType.PREFAB);
