...
public class RuneEquipWindow : BaseWindow {
    ...
    public void onClickCombine() { RuneCombineCtrl.Instance.Enter(); }

    public void onClickWash() { EventCenter.Broadcast(EGameEvent.eGameEvent_RuneRefeshWindowEnter); }
    ...
    public void onAddPageClick() { MsgInfoManager.Instance.ShowMsg(10043); }
    ...
    public void OnCloseInterface() { Hide(); }
    ...
    private void AddRes(int i) {
        {
            ...
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.RuneSlotAddFlash, ResourceType.PREFAB);
    ...
    public void RefeshEquipRuneInfoWhenInit(uint runeid, int sPage, int slotPos) {
...

        {
            ...
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.RuneSlotEquipLight, ResourceType.PREFAB);
            objLoad = GameObject.Instantiate(objUnit.Asset) as GameObject;
            objLoad.transform.parent = transFrom;
            objLoad.transform.localScale = Vector3.one;
            objLoad.transform.localPosition = Vector3.zero;
    ...
    private void LoadOneRune(uint runeid, int num) {
        ...
        if (isNewRune) {
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadGameMarketRuneItem, ResourceType.PREFAB);
        ...
        foreach (var kv in addFlashMap) {
            if (Time.time - kv.Value >= 0.75) {
...

                {
                    ...
                    ResourceUnit objUnit = ResourcesManager.Instance.loadImmediate(GameConstDefine.RuneSlotEquipLight,
                                                                                   ResourceType.PREFAB);
                    objLoad = GameObject.Instantiate(objUnit.Asset) as GameObject;
                    objLoad.transform.parent = transFrom;
                    objLoad.transform.localScale = Vector3.one;
    ...
    private uint GetEffectKey(uint runeID) { return runeID % 100; }
    ...
    public class RuneSlotInfo {
        public int slotPos { set; get; }
        public int runeID { set; get; }
    }
    ...
    public Dictionary<uint, nowAllRuneGOCache> nowAllRuneGO =
        new Dictionary<uint, nowAllRuneGOCache>();  // 当前等级存在的所有符文!等级不符合的全部删除！！
    ...
    public class SlotInfo {
        public bool ifCanEquip { set; get; }
        public UISprite sprite { set; get; }
        public uint runeid { set; get; }
        public GameObject lockGO { set; get; }

        public GameObject resGO1 { set; get; }
        public GameObject resGO2 { set; get; }
    }
}
}
