...
public class BattleCtrl : Singleton<BattleCtrl> {
    public void Enter() { EventCenter.Broadcast(EGameEvent.eGameEvent_BattleEnter); }

    public void Exit() { EventCenter.Broadcast(EGameEvent.eGameEvent_BattleExit); }

    // 创建房间
    public void CreateRoom(int mapId, string password) { CGLCtrl_GameLogic.Instance.CreateRoom(mapId, password); }

    // 向服务器请求刷新房间列表
    public void AskRoomList() { CGLCtrl_GameLogic.Instance.AskRoomList(); }
    ...
    public void UpdateRoomList(List<GSToGC.RoomInfo> roomList) {
        ...
        foreach (GSToGC.RoomInfo roomInfo in roomList) {
            AddRoomItem((UInt32)roomInfo.roomId,
                        (UInt32)roomInfo.mapId,
                        roomInfo.master,
                        (UInt32)roomInfo.curUserCount,
                        (UInt32)roomInfo.maxUserCount,
                        (roomInfo.ifPwd == 1));
        }
    ...
    // 取得房间列表
    public Dictionary<UInt64, RoomItem> GetRoomList() { return mRoomList; }
