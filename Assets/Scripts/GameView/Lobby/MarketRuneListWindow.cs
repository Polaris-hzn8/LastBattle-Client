...
public class MarketRuneListWindow : BaseWindow {
    ...
    // 窗口控件释放
    protected override void RealseWidget() { runeListObjects.Clear(); }
    ...
    // 游戏事件注册
    protected override void OnAddListener() { EventCenter.AddListener(EGameEvent.eGameEvent_LobbyExit, Hide); }

    // 游戏事件注消
    protected override void OnRemoveListener() { EventCenter.RemoveListener(EGameEvent.eGameEvent_LobbyExit, Hide); }
    ...
    protected void OnShowLevel() { m_RuneLevelGo.SetActive(true); }

    protected void OnMagicKind() {}

    public void onClickRuneCard(GameObject go) { MarketRuneInfoCtrl.Instance.Enter(go); }
    ...
    private void LoadOneRune(RuneGoodsInfo val, RuneConfigInfo sRuneConfigInfo) {
        ResourceUnit objUnit =
            ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadGameMarketRuneTag, ResourceType.PREFAB);
