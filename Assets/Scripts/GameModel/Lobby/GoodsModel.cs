...
public class GoodsModel : Singleton<GoodsModel> {
    ...
    public Dictionary<int, RuneBaptze> GetRuneBaptzeMap() { return m_RuneBaptze; }
    ...
    public void AddOrChangeRuneBaptze(List<GSToGC.NotifyOtherItemInfo.OtherItem> list) {
        foreach (var item in list) {
            ...
            EventCenter.Broadcast<uint, int, long>(
                EGameEvent.eGameEvent_RuneRefreshCardUpdate, item.item_id, (int)item.item_num, (long)item.expired_time);
        }
    }
}
