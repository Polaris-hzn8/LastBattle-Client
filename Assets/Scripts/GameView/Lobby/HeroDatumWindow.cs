...
public class HeroDatumWindow : BaseWindow {
    ...
    private void CloseHeroDatum() { HeroDatumCtrl.Instance.Exit(); }
    ...
    public override void OnEnable() { ShowHeroDatum(HeroType.AllHero); }
    // 隐藏
    public override void OnDisable() { isOpen = false; }
    ...
    private void InitHeroHead() {
        ...
        foreach (int hero in HeroList) {
            ResourceUnit objHomeBaseUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadHeroDataPhoto, ResourceType.PREFAB);
