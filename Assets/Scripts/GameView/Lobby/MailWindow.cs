...
public class MailWindow : BaseWindow {
    ...
    protected override void InitWidget() {
        ...
        curMailObj.mMailCreateTimeTxt =
            mRoot.Find("MailIntroduction/Scroll View/Table/PublishDate").GetComponent<UILabel>();
    ...
    // 窗口控件释放
    protected override void RealseWidget() { Clear(); }
        ...
        if (mailList != null && mailList.Count > 0) {
            foreach (var kv in mailList) {
                ...
                ResourceUnit objUnit =
                    ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadMailTitleGUI, ResourceType.PREFAB);
                ...
                if ((LastObj == null) || (LastObj.currMailId <= mail.currMailId &&
                                          kv.Value.mailCurtState == EMailCurtState.eMailState_New)) {
                    LastObj = mail;
                    curtActiveObj = obj;
                }
                mail.objParent = obj;
    ...
    public void UpdateMail(MailData mdb) {
        ...
        if ((curMailObj != null) && (mdb != null) && (curtMailId == mdb.MailId)) {
            ...
            for (int i = 0; i < giftArray.Length; i++) {
                ...
                ResourceUnit objUnit =
                    ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadMailAttachGUI, ResourceType.PREFAB);
                    ...
                    switch (giftType) {
                        ...
                        default: {
                            if (goodsId >= 100001 && goodsId <= 109999) {
                                ...
                                if (buyInfo != null) {
                                    ResourceUnit GetMoneyUnit = ResourcesManager.Instance.loadImmediate(
                                        GameConstDefine.LoadDailyHeroIcon, ResourceType.PREFAB);
