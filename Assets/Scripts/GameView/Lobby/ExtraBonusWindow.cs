...
public class ExtraBonusWindow : BaseWindow {
    ...
    private void ClosePress() { ExtraBonusCtrl.Instance.Exit(); }

    private void ExchangePress() {
        if (mInputKey.value.Trim() == null || string.IsNullOrEmpty(mInputKey.value.Trim()) ||
            string.Compare(mInputKey.value.Trim(), "") == 0) {
