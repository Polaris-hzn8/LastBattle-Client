...
public class BattleWindow : BaseWindow {
    ...
    protected override void InitWidget() {
        ...
        mSearchPassWordInput =
            mRoot.Find("CustomInterface/JoinBattles/SearchInterface/Password").GetComponent<UIInput>();
    ...
    // 自定义选择地图
    private void OnCustomSelectMap(GameObject obj) { mMapId = Convert.ToInt32(obj.name); }
    ...
    // 刷新房间列表
    public void OnRefresh() { BattleCtrl.Instance.AskRoomList(); }

    // 加入房间
    public void OnAddJoinRoom() { BattleCtrl.Instance.AskAddRoom(mRoomID.ToString(), mPasswordInput.text); }

    // 加入房间返回
    public void OnAddBack() { mPasswordPanel.gameObject.SetActive(false); }
    ...
    // 查找返回
    public void OnSearchBack() { mSearchInterface.gameObject.SetActive(false); }
