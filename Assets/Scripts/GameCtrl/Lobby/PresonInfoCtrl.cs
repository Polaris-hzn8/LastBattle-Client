...
public class PresonInfoCtrl : Singleton<PresonInfoCtrl> {
    public void Enter() { EventCenter.Broadcast(EGameEvent.eGameEvent_PresonInfoEnter); }

    public void Exit() { EventCenter.Broadcast(EGameEvent.eGameEvent_PresonInfoExit); }

    public void ChangeNickName() { EventCenter.Broadcast(EGameEvent.eGameEvent_ChangeNickName); }
    public void ChangeHeadID() { EventCenter.Broadcast(EGameEvent.eGameEvent_ChangeHeadID); }
    public void SetHeadID(ulong sGUID, uint headID) {
        GameUserModel.Instance.SetGameUserGUID(sGUID);
        mSGUID = sGUID;
        GameUserModel.Instance.SetGameUserHead((int)headID);
    }

    public void ChangeHead(uint headID) { CGLCtrl_GameLogic.Instance.EmsgToss_AskChangeheaderId(headID); }

    public void ChangeNickName(string nickName) { CGLCtrl_GameLogic.Instance.EmsgToss_AskChangeNickName(nickName); }
    ...
    public void SetCurrentDate(int headID,
                               string pszNickName,
                               int UserLv,
                               int UserTotalExp,
                               int UserCurLvExp,
    ...
    int GetSecond(ulong time) { return (int)time % 60; }

    public int GetMinute(ulong time) { return (int)time / 60 % 60; }

    int GetHour(ulong time) { return (int)time / 3600; }
    ...
    public bool GetTimeExp() { return isTimeExtRun; }
    public bool GetTimeGold() { return isTimeGoldRun; }
    public void SetTimeExp(bool isTrue) { isTimeExtRun = isTrue; }
    public void SetTimeGold(bool isTrue) { isTimeGoldRun = isTrue; }
}
}
