...
public class MiniMapWindow : BaseWindow {
    ...
    // 显示
    public override void OnEnable() { mInited = false; }
    ...
    private Dictionary<UInt64, UIMiniMapInterfaceWarning> mMapWarningDic =
        new Dictionary<UInt64, UIMiniMapInterfaceWarning>();
    ...
    private void AddMapWarning(UInt64 guid) {
        ...
        if (SceneGuideTaskManager.Instance().IsNewsGuide() != SceneGuideTaskManager.SceneGuideType.NoGuide &&
            (entity.NpcGUIDType == 21017 || entity.NpcGUIDType == 21025 || entity.NpcGUIDType == 21024)) {
            isAllow = true;
        }
        if (!isAllow)
            return;
    ...
    private void Clear() {
        for (int i = mMapElementDic.Count - 1; i >= 0; i--) {
            ...
            GameObjectPool.Instance.ReleaseGO(mMapElementDic.ElementAt(i).Value.resPath,
                                              mMapElementDic.ElementAt(i).Value.gameObject,
                                              PoolObjectType.POT_MiniMap);
        }
        mMapElementDic.Clear();
    }
    ...
    private void OnAddMapEvent(Ientity entity) {
        ...
        if (entity.entityType != EntityType.Player && entity.entityType != EntityType.Soldier &&
            entity.entityType != EntityType.AltarSoldier && entity.entityType != EntityType.Building &&
    ...
    private void OnRemoveMapEvent(UInt64 guid) { DestroyMapElement(guid); }
}
}