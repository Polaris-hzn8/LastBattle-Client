...
public class SystemNoticeData {
    ...
    string value =
        "[d2853e]最后一战精英测试公告[-]\n[ffe9db]亲爱的小伙伴们:\n " +
        "最后一战精英测试已经隆重拉开帷幕，我们真切地邀请各位小伙伴们参与该次测试并把你们对最后一战的期望和建议反馈给" +
        "我们。我们也准备了大量精美的礼品送给积极反馈的你们。祝各位游戏愉快!\n[58e901]测试时间：2014年10月18日-" +
        "10月25日[-]反馈请发送至邮箱：bl@skymoon.com\n[00fffc]最后一战运营团队[-]";
    internal void SetSystem() {
        // for (int i = 0; i < 10; i++)
        //{
        //     SetSystemNotList("title_" + i, (NoticeIdentify)Mathf.Abs(i % 3), (NoticeState)Mathf.Abs(i / 2), i,i +
        //     value);
        // }
    }
    public void Clear() {
    ...
    public class SystemNoticeType {
        ...
        public int mNoticChildSort { private set; get; }

        public string mNoticeContent  // 公告内容
            { private set; get; }

        public void SetNoticChildSort(int childSort) { mNoticChildSort = (++childSort); }

        public void SetSystemNotice(
            string title, NoticeIdentify identify, NoticeState state, int sort, string content) {
            mNoticChildSort = 0;
            mNoticeTitle = title;
