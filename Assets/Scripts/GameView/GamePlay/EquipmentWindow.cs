...
public class EquipmentWindow : BaseWindow {
    ...
    protected override void InitWidget() {
        ...
        for (int i = 0; i < 3; i++) {
            mSpriteBackPack.Add(
                mBackPackage.Find("ItemPack" + (i + 1).ToString()).Find("icon").GetComponent<UISprite>());
    ...
    void ShowBlank(BlankShowState state) {
        ...
        switch (state) {
            case BlankShowState.CloseState:
                Vector3 pos =
                    new Vector3(mOrginalPos.x, mOrginalPos.y - (mYOffset * mAdjust.localScale.x), mOrginalPos.z);
    ...
    void ShowBackPackage(bool show) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (PlayerManager.Instance.LocalPlayer != null &&
                PlayerManager.Instance.LocalPlayer.UserGameItemsCount != null &&
                PlayerManager.Instance.LocalPlayer.UserGameItemsCount.Count > (showStart + i)) {
    ...
    void ShowItemCost(int index) {
        ...
        if (mItemCdDic.ElementAt(index).Value.cdSlider.gameObject.activeInHierarchy) {
            ...
            if (count == 0 || !ConfigReader.ItemXmlInfoDict.ContainsKey(itemId) ||
                ConfigReader.ItemXmlInfoDict[itemId].un8UseTimes == 0) {
    ...
    private void OnSelectItemToUse(GameObject obj) {
        ...
        PlayerManager.Instance.LocalPlayer.UserGameItems.TryGetValue(start, out itemId);
        if (cdCool > 0f && count != 0) {  // 如果对应背包物品为空，或者个数为0，或者正在CD不发送使用请求
    ...
    void ShowSelect(int index, bool show) {  // 0~3
        int count = 0, itemId = -1;
        float cdTime = 0f;
        ...
        if (!mItemSelect.TryGetValue(index, out spriteSelect))
            return;
        if (PlayerManager.Instance.LocalPlayer != null) {
