...
public class AudioManager {
    ...
    public AudioSource LoopAudioSource { private set; get; }

    public AudioSource HeroMoveAudio { private set; get; }

    public AudioSource GuideVoice { private set; get; }

    public Queue<AudioSource> EffectAudioSourceQueue { private set; get; }

    public Queue<AudioSource> LongVoiceAudioSourceQueue { private set; get; }

    /// <summary>
    /// 各英雄台词播放
    /// </summary>
    public Dictionary<UInt64, AudioSource> HeroLinesAudioDict { private set; get; }

    /// <summary>
    /// 游戏击杀音效播放列表
    /// </summary>
    private List<AudioClip> GameKillClipList { set; get; }

    public AudioSource GameKillAudioSource { private set; get; }

    public AudioSource HeroGetMoneySource { private set; get; }

    private AudioManager() { InitAudio(); }
    ...
    public void PlayGameKillAudio(AudioClip adClip) { GameKillClipList.Add(adClip); }
    ...
    public void GetMoneyAudio() {
        if (GetMoney == null) {
            ...
            ResourceUnit GetMoneyUnit =
                ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_GETMONEY, ResourceType.ASSET);
    ...
    public void PlayBgAudio(AudioClip adClip) {
        if (LoopAudioSource == null || adClip == null ||
            (LoopAudioSource.clip != null && LoopAudioSource.clip.name == adClip.name)) {
            return;
        }
    ...
    public void EnableVoice(bool enable) {
        ...
        if (enable) {
            ResourceUnit audioClipUnit = ResourcesManager.Instance.loadImmediate(
                AudioDefine.GetMapBgAudio((MAPTYPE)GameUserModel.Instance.GameMapID), ResourceType.ASSET);
