...
#region 加载选择英雄显示预设
            ...
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.HeroShowBoxUI, ResourceType.PREFAB);
...
#region 获得英雄头像预设
        ...
        for (int j = 0; j < mHeroCount; j++) {
            ...
            if (j % 2 == 0) {  // 左边三个 j = 0,2,4 , i = 1,2,3
                teamParent = mTeamLeft;
            } else {  // 右边三个 j = 1,3,5, i = 1,2,3
...
#region 倒计时
    ...
    public override void OnDisable() {
        // UIGuide
        IGuideTaskManager.Instance().RemoveTaskStartListerner(EGameEvent.eGameEvent_UIGuideSelectHeroHeadStart,
                                                              StartIGuideTask);
        IGuideTaskManager.Instance().RemoveTaskStartListerner(EGameEvent.eGameEvent_UIGuideSelectHeroCommitStart,
                                                              StartIGuideTask);
...
#region 预选择英雄显示
    public void UpdateHeroListSelect() {  // 刷新选择列表
        ...
        foreach (var item in HeroCtrl.Instance.heroSelectDic.Keys) {
            ...
            if (!HeroCtrl.Instance.heroSelectDic.TryGetValue(item, out heroId))
                continue;
            if (item.GameUserSeat != mLocalPlayer.GameUserSeat &&
                mLocalPlayer.IsSameCamp(item)) {  // 找到队友所选择的英雄
                int index = GetSelectIndexBuyHeroId(heroId);
                mSpriteBlackList[index].gameObject.SetActive(true);
            }

            mSpriteDic[(int)item.GameUserSeat - 1].spriteName =
                ConfigReader.GetHeroSelectInfo(heroId).HeroSelectHead.ToString();
...
#region 更新左右两边队伍是否选择英雄的显示
    public void UpdateSureSelect() {  // 刷新左右两边队伍是否已经选择
...
#region 进入选择英雄阶段
    private void EnterSelectHero() {
        ...
        // UIGuide
        IGuideTaskManager.Instance().AddTaskStartListerner(EGameEvent.eGameEvent_UIGuideSelectHeroHeadStart,
                                                           StartIGuideTask);
        IGuideTaskManager.Instance().AddTaskStartListerner(EGameEvent.eGameEvent_UIGuideSelectHeroCommitStart,
                                                           StartIGuideTask);
        IGuideTaskManager.Instance().SendTaskTrigger(EGameEvent.eGameEvent_UIGuideTriggerSelectHero);
    }
#endregion

    void StartIGuideTask(EGameEvent taskId) { IGuideTaskManager.Instance().SendTaskEffectShow(taskId); }
...
#region 点击确定选择英雄按钮事件
    private void OnCommitHero(int ie, bool press) {
        ...
        if (mSelectIndex != -1) {
            IGuideTaskManager.Instance().SendTaskEnd(EGameEvent.eGameEvent_UIGuideSelectHeroCommitEnd);
            CGLCtrl_GameLogic.Instance.EmsgToss_AskSelectHero(
                (UInt32)HeroCtrl.Instance.heroInfoList.ElementAt(mSelectIndex).HeroSelectNum);
...
#region 通过英雄ID号获得选择的是第几个英雄
    ...
    public void SetSelectShow(int index) { mHeroShowList[index].value = true; }
...
#region 通过英雄ID判断队友是否有选择此英雄
    ...
    public void SetFirstTime(int time) { mFirstCountTime = time; }

    public void SetSecondTime(int time) { mSecondCountTime = time; }
    ...
    // 这里出发发送服务器记录的ui点击事件消息
    public void SendUIEventMsg() { CGLCtrl_GameLogic.Instance.EmsgTocs_ReportUIEvent(); }
}
}