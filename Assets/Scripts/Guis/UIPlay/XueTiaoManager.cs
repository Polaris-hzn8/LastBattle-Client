...
#region 血条路径
    ...
    public static XueTiaoManager Instance { private set; get; }
    ...
    private string GetXueTiaoPrefabPath(Ientity entity) {
        ...
        } else if (entity.EntityCamp == EntityCampType.CampTypeBad ||
                   (PlayerManager.Instance.LocalPlayer != null &&
                    entity.EntityCamp != PlayerManager.Instance.LocalPlayer.EntityCamp)) {
            XuetiaoColor = "Red";
        }

        if (PlayerManager.Instance.LocalAccount.ObType == GameDefine.ObPlayerOrPlayer.PlayerObType &&
            entity.EntityCamp != PlayerManager.Instance.LocalAccount.EntityCamp) {
            XuetiaoColor = "Red";
        }
        string path = "";
        ...
        if (entity.NpcGUIDType == 21017 || entity.NpcGUIDType == 21025 ||
            entity.NpcGUIDType == 21024) {  // 新手引导假的英雄
            path = String.Format(pathPlayerXuetiao, XuetiaoColor);
