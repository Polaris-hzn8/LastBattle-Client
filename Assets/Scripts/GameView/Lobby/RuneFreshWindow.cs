...
public class RuneRefreshWindow : BaseWindow {
    ...
    public class RefreshCardInfo {
        ...
        public OtherItemConfigInfo cfg { set; get; }
    }
    ...
    public Dictionary<uint, RefreshCardInfo> refreshCardListID2GoDict =
        new Dictionary<uint, RefreshCardInfo>();  // 所有符文！
    public Dictionary<uint, nowAllRuneGOCache> nowAllRuneGO =
        new Dictionary<uint, nowAllRuneGOCache>();  // 当前等级存在的所有符文!等级不
    ...
    protected override void InitWidget() {
        ...
        UIEventListener.Get(m_Slot_SpriteArray[GameDefine.GameConstDefine.RefreshRuneSlotIndex].gameObject).onClick +=
            onClickSlot;
        UIEventListener.Get(m_Slot_SpriteArray[GameDefine.GameConstDefine.RefreshRuneCardIndex].gameObject).onClick +=
            onClickRefreshSlot;
    ...
    private void LoadOneRefreshCard(uint refreshID, int num) {
        ...
        ResourceUnit objUnit =
            ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadRuneRefreshCardRes, ResourceType.PREFAB);
    ...
    public void onClickWash() {
        ...
        EventCenter.Broadcast<EMessageType, string, Callback<bool>>(
            EGameEvent.eGameEvent_ShowLogicMessage, EMessageType.EMT_SureToWash, showStr, windowsCallBack);
    }
