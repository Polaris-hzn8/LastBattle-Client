...
public class BaseDaBomb : MonoBehaviour {
    public static BaseDaBomb Instance { set; get; }
        ...
        if (isBombBase) {
            ...
            if (move == transform.position && Time.time - startTime >= 1f) {
                ...
                if (move == posDest) {
                     ...
                     */
                    // soul =
                    // BlGame.AudioManager.Instance.PlayEffectAudio(Resources.Load(AudioDefine.PATH_GAME_OVER_FAILE) as
                    // AudioClip);

                    ResourceUnit clipUnit =
                        ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_GAME_OVER_FAILE, ResourceType.ASSET);
                    AudioClip clip = clipUnit.Asset as AudioClip;

                    soul = BlGame.AudioManager.Instance.PlayEffectAudio(clip);
                } else {
                    // soul =
                    // BlGame.AudioManager.Instance.PlayEffectAudio(Resources.Load(AudioDefine.PATH_GAME_OVER_VICTORY)
                    // as AudioClip);

                    ResourceUnit clipUnit =
                        ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_GAME_OVER_VICTORY, ResourceType.ASSET);
                    AudioClip clip = clipUnit.Asset as AudioClip;
            ...
            // Camera.main.transform.position = Vector3.Lerp(Camera.main.transform.position, move,Time.deltaTime *
            // time);
        }
    }
