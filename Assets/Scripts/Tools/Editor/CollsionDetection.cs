...
#region 创建碰撞体
    public static void createCollsionObject(float map_width, float map_height, string layer_name) {
        ...
        int tag = 0;
        int layer = 1 << LayerMask.NameToLayer(layer_name);
        GameObject obj_parent = UnityEngine.Object.Instantiate(
            Resources.Load("Cube/Cubebb"), Vector3.zero, Quaternion.identity) as GameObject;
        for (int i = 0; i < map_height; i++) {
            for (int j = 0; j < map_width; j++) {
                float a = Convert.ToSingle(j / 2.00f + 0.25f);
                float b = Convert.ToSingle(i / 2.00f + 0.25f);
                ...
                if (isBunker) {
                    tag = 1;
                    GameObject tobj = UnityEngine.Object.Instantiate(
                        Resources.Load("Cube/Cubebb"), pos, Quaternion.identity) as GameObject;
