...
#region 设置蓝条
    public void SetMp(float mp) { mpSprite.fillAmount = mp; }
#endregion

#region 更新蓝条
    public void UpdateMp() { mpSprite.fillAmount = xueTiaoOwner.Mp / xueTiaoOwner.MpMax; }
...
#region 玩家血条缓慢扣除
    public override void XueTiaoDmgShow(float currentHp, float dmg, float maxHp) {
        ...
        if (hideSlider != null && hideSprite != null && dmg > 0) {
            Vector3 currentPos = new Vector3(
                hpSprite.width * value, hideSlider.transform.localPosition.y, hideSlider.transform.localPosition.z);
    ...
    void OnDisable() { hideSprite.fillAmount = 0f; }
...
#region 更新血条信息
    ...
    void ShowGuideXueTiaoInfo() {
        if ((SceneGuideTaskManager.Instance().IsNewsGuide() != SceneGuideTaskManager.SceneGuideType.NoGuide) &&
            (xueTiaoOwner.entityType == EntityType.Soldier || xueTiaoOwner.entityType == EntityType.AltarSoldier)) {
