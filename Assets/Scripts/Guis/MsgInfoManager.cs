...
public class MsgInfoManager {
    ...
    public void SetKillDeathPlayer(
        Byte killstate, int campID, UInt64 targetID, UInt64 killID, bool Aced, EHeroKillTitle Title) {
    ...
    public void SetKills(eKillMsgType killType,
                         bool isaced,
                         string nameKill,
                         string nameDeath,
                         string readXml,
                         EHeroKillTitle Title = EHeroKillTitle.eNoneTitel) {
    ...
    void SetKillShow() {
        if (isPlayEffect && KillMsgQueue != null && KillMsgQueue.Count > 0) {
            ...
            data = temp;
            temp.msgkill.Init();
            temp.msgkill.ShowMsgInfo(
                temp.KillsType, temp.isAced, temp.NameKill, temp.NameDeath, temp.ReadXml, temp.Title);
        }
    }
    ...
    public class MsgKill {
        ...
        public void ShowMsgInfo(
            eKillMsgType type, bool isaced, string namekill, string namedeath, string readXml, EHeroKillTitle Title) {
    ...
    class Msg {
        ...
        void ShowInfoTweenRotation() {
            ...
            UITweener tween =
                TweenRotation.Begin(root, SettingEnd.rotation_time_out, Quaternion.Euler(SettingEnd.rotation_end_out));
            tween.delay = SettingEnd.stayTime;
            DoTweenEnd(tween);
        }
        ...
        void ShowInfoTweenAlpha() {
            ...
            UITweener tween =
                TweenAlpha.Begin(Content.gameObject, SettingEnd.alpha_time_out, SettingEnd.alpha_end_out / Alpha);
            tween.delay = SettingEnd.stayTime;
            DoTweenEnd(tween);
        }
            ...
            switch (audiotype) {
                ...
                case AudioPlayType.TwentySconde: {
                    ...
                    if (randomdata == 1) {
                        ...
                        // int int_hero1 = int.Parse(info.HeroScript1Rate.Substring(0,
                        // info.HeroScript1Rate.IndexOf(","))); if (audio_num < (int_hero1 % 90000)) adio_name =
                        // info.HeroScript1.Substring(0, info.HeroScript1.IndexOf(",")); else adio_name =
                        // info.HeroScript1.Substring(info.HeroScript1.IndexOf(",") + 1, info.HeroScript1.Length -
                        // info.HeroScript1.IndexOf(",") - 1);

                        // AudioClip clip = Resources.Load(path + adio_name) as AudioClip;
                        // AudioManager.Instance.PlayGameKillAudio(clip);

                        ResourceUnit clipUnit =
                            ResourcesManager.Instance.loadImmediate(path + adio_name, ResourceType.ASSET);
                        AudioClip clip = clipUnit.Asset as AudioClip;
                ...
                case AudioPlayType.FuhuoAudio: {
                    if (info.HeroScript1 != null) {
                        string path = "Audio/HeroSelect/";
                        string adio_name =
                            info.HeroScript1.Substring(info.HeroScript1.IndexOf(",") + 1,
                                                       info.HeroScript1.Length - info.HeroScript1.IndexOf(",") - 1);
