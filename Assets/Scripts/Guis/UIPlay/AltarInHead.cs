...
public class AltarInHead : MonoBehaviour {
    public static AltarInHead Instance { set; get; }
    ...
    public void CreateHead(int type) {
        if (head == null) {
            head = ((GamePlayWindow)WindowManager.Instance.GetWindow(EWindowType.EWT_GamePlayWindow))
                       .CreateAltarUIPrefab(this, type);
        } else
