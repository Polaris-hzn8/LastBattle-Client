...
public class GamePlayCtrl : Singleton<GamePlayCtrl> {
    public void Enter() { EventCenter.Broadcast(EGameEvent.eGameEvent_GamePlayEnter); }

    public void Exit() { EventCenter.Broadcast(EGameEvent.eGameEvent_GamePlayExit); }

    public void AskBattleInfo() { CGLCtrl_GameLogic.Instance.EmsgToss_AskBattleInfo(); }
    ...
    internal void AskUseSkill(uint p) { CGLCtrl_GameLogic.Instance.EmsgToss_AskUseSkill((UInt32)p); }
    ...
    internal void SetShopID(int p) { ShopID = p; }
