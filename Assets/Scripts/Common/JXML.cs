...
public static implicit operator bool(JString xs) {
    if (xs.value == "" || xs.value.ToLower() == "false" || xs.value == "0")
        return false;
    return true;
}
}
public class JNode {
    ...
    public JNode() { Attributes = new JAttribute(this); }
    ...
    public JNode this[string NodeName] {
        get {
            return new JNode(this, NodeName);
        }
    }

    public JNode this[int NodeIndex] {
        get {
            return new JNode(this, NodeIndex);
        }
    }
}
...
public class JXml : JNode {
    XmlDocument xml;
    public JXml(string xmlstring, bool hasDocumentElement = true) : base() {
        xml = new XmlDocument();
        xml.LoadXml(xmlstring);
        node = hasDocumentElement ? (XmlNode)xml.DocumentElement : xml;
    }
    public void SaveToFile(string fileName) { System.IO.File.WriteAllText(fileName, xml.OuterXml); }
...
public class JAttribute {
    JNode node;
    public JAttribute(JNode xn) { node = xn; }
    ...
    public JString this[string AttributeName] {
        get {
            return (node.GetXmlNode(false) == null || node.GetXmlNode(false).Attributes[AttributeName] == null)
                       ? ""
                       : node.GetXmlNode(false).Attributes[AttributeName].Value;
        }
        set {
