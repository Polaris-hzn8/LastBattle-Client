...
public class RoomWindow : BaseWindow {
    ...
    protected override void InitWidget() {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            UISprite parentReady =
                mBtnStart.transform.Find("Ready/Ready" + (i + 1).ToString()).GetComponent<UISprite>();
            mReadySprite.Add(parentReady);
            UISprite parentStart =
                mBtnStart.transform.Find("Start/Start" + (i + 1).ToString()).GetComponent<UISprite>();
    ...
    public UIButton InvaitaionBtn { set; get; }
    ...
    void InitEffect() {
        ...
        mTteamObj[0].transform.localPosition = new Vector3(mTteamObj[0].transform.localPosition.x - mXOffset,
                                                           mTteamObj[0].transform.localPosition.y,
                                                           mTteamObj[0].transform.localPosition.z);
        TweenPosition.Begin(mTteamObj[0],
                            mMoveDuring,
                            new Vector3(mTteamObj[0].transform.localPosition.x + mXOffset,
                                        mTteamObj[0].transform.localPosition.y,
                                        mTteamObj[0].transform.localPosition.z));
        mTteamObj[1].transform.localPosition = new Vector3(mTteamObj[1].transform.localPosition.x + mXOffset,
                                                           mTteamObj[1].transform.localPosition.y,
                                                           mTteamObj[1].transform.localPosition.z);
        TweenPosition.Begin(mTteamObj[1],
                            mMoveDuring,
                            new Vector3(mTteamObj[1].transform.localPosition.x - mXOffset,
                                        mTteamObj[1].transform.localPosition.y,
                                        mTteamObj[1].transform.localPosition.z));
        ...
        // UIGuide
        IGuideTaskManager.Instance().AddTaskStartListerner(EGameEvent.eGameEvent_UIGuideRoomBeginBtnStart,
                                                           StartIGuideTask);
        IGuideTaskManager.Instance().SendTaskTrigger(EGameEvent.eGameEvent_UIGuideTriggerRoomBeginGame);

        IGuideTaskManager.Instance().AddTaskStartListerner(EGameEvent.eGameEvent_UIGuideBackLobbyBtnStart,
                                                           StartIGuideTask);
...
#region 显示房间Id
    void ShowRoomId() { mRoomNumLabel.text = "房间号： " + GameUserModel.Instance.GameRoomID.ToString(); }
#endregion
...
#region 控制按钮处理
    ...
    void BeginBattleError(EErrorCode m_n32ErrorId) { MsgInfoManager.Instance.ShowMsg((int)m_n32ErrorId); }
#region 座位处理
    ...
    void BtnEvent(int ie, bool pressed) {
        ...
        switch ((GameRoomBtn)ie) {
            ...
            case GameRoomBtn.LEAVEBATTLE:
                ...
                if (!PlayerManager.Instance.LocalAccount.isGameLookViewer() &&
                    PlayerManager.Instance.LocalAccount.IsReady() && !PlayerManager.Instance.LocalAccount.IsMaster()) {
    ...
    public void ShowRoomEnd() {
        ...
        foreach (var item in PlayerManager.Instance.AccountDic.Values) {
            if (item.GameUserSeat <= SeatTeam) {
                ...
                UITweener tween =
                    TweenPosition.Begin(mSoulObj[index], mTweenTime, mBallObj.transform.position + mSoulTargetPos);
