...
public class MarketHeroInfoWindow : BaseWindow {
        ...
        if (MarketHeroListModel.Instance.HeroListDict.TryGetValue(goodsId, out mGoodInfo) && isEnable) {
            if (mGoodInfo.RLDiamondPrice != 0 && mGoodInfo.RLGoldPrice != 0) {
                ...
                if (heroInfo == null)
                    return;
                bool isVib =
                    GameUserModel.Instance.OwnedHeroInfoDict.ContainsKey(MarketHeroListCtrl.Instance.GetRootGoods())
                        ? false
                        : true;
    ...
    void OnBuyByGold() {
        ...
        MarketHeroListCtrl.Instance.MarketHeroAskBuyGoods(MarketHeroListCtrl.Instance.GetGoodsSelect(),
                                                          GameDefine.ConsumeType.TypeGold);
    }
    ...
    void OnBuyByDiamond() {
        ...
        MarketHeroListCtrl.Instance.MarketHeroAskBuyGoods(MarketHeroListCtrl.Instance.GetGoodsSelect(),
                                                          GameDefine.ConsumeType.TypeDiamond);
    }
    ...
    public void OnCloseInterface() { MarketHeroInfoCtrl.Instance.Exit(); }
