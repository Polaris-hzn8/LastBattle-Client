...
public class DeathWindow : BaseWindow {
    ...
    private void MoveMian(GameObject go, Vector2 delta) {
        ...
        if (camInfo != null) {
            Vector3 v1 =
                new Vector3(GameMethod.GetMainCamera.transform.position.x - (delta.x / camInfo.mMovementSpeed),
                            GameMethod.GetMainCamera.transform.position.y,
                            GameMethod.GetMainCamera.transform.position.z - (delta.y / camInfo.mMovementSpeed));
    ...
    private void BtnOnPressRecovery() { DeathCtrl.Instance.SendBtn(); }
    ...
    public override void OnEnable() {
        ...
        if (money == 0) {
            ...
            RecoveryLabel.transform.localPosition = new Vector3(RecoveryLabel.transform.localPosition.x + 30,
                                                                RecoveryLabel.transform.localPosition.y,
                                                                RecoveryLabel.transform.localPosition.z);
        }
    ...
    // 游戏事件注册
    protected override void OnAddListener() { EventCenter.AddListener(EGameEvent.eGameEvent_HeroReborn, ExitDeath); }
