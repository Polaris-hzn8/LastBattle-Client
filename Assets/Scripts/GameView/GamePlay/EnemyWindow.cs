...
public class EnemyWindow : BaseWindow {
    enum EnemyInfo { ENEMY_ONE, ENEMY_TWO, ENEMY_THR }
    ...
    // 窗口控件释放
    protected override void RealseWidget() { mEnemyTeamList.Clear(); }
    ...
    public override void Update(float deltaTime) {
        ...
        foreach (var item in mEnemyTeamList) {
            ...
            float dis =
                Vector3.Distance(item.Player.realObject.transform.position, player.realObject.transform.position);
            if (item.Player.FSM.State == BlGame.FSM.FsmState.FSM_STATE_DEAD ||
                dis > GameConstDefine.PlayerLockTargetDis || item.Player.realObject.activeSelf == false) {
                item.CloseInfo();
            } else {
                item.OnUpdateHp();
...
public class EnemyHeadInfo {
    ...
    public void OnSelect(bool isVis) { SelectSprite.gameObject.SetActive(isVis); }
    ...
    public void CloseInfo() {
        Player = null;
        uit = TweenPosition.Begin(
            teamPress.gameObject, 0.5f, new Vector3(orignalPos.x + 110f, orignalPos.y, orignalPos.z));
        SelectSprite.gameObject.SetActive(false);
