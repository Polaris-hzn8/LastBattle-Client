...
public class ResourceCommon {
    ...
    public static void getFiles(
        string path, bool recursion, Dictionary<string, List<string>> allFiles, bool useSuffix, string suffix) {
        if (recursion) {
    ...
    public static string getServerPath(string path) { return string.Format("{0}/{1}", mServerAssetPath, path); }
    ...
    public static IEnumerator _CreateFromMemory(string path, Handle_CreateFromMemory handle) {
        ...
        AssetBundleCreateRequest bundleRequest =
            AssetBundle.LoadFromMemoryAsync(ResourceCommon.getAssetBundleFileBytes(path, ref size));
        yield return bundleRequest;
        handle(bundleRequest, size);
    }

    public delegate void HandleFinishLoadAsyncFromAssetBundle(AssetBundleRequest request);
    public static IEnumerator LoadAsyncFromAssetBundle(AssetBundle assetbundle,
                                                       string name,
                                                       System.Type type,
                                                       HandleFinishLoadAsyncFromAssetBundle handle) {
    ...
    public static AssetBundle loadAssetBundleImmediateForDebug(string path) {
        string localPath =
            string.Format("file://{0}/{1}", assetbundleFilePath, path + ResourceCommon.assetbundleFileSuffix);
