...
public class CPAddUI : MonoBehaviour {
    ...
    public void SetCPAdd_Count(string count, GameObject temp) {
        ...
        BlGame.Effect.EffectManager.Instance.CreateNormalEffect(
            "effect/other/soul_death", PlayerManager.Instance.LocalPlayer.RealEntity.objAttackPoint.gameObject);
        TweenPosition.Begin(temp.gameObject,
                            0.8f,
                            new Vector3(temp.transform.localPosition.x,
                                        temp.transform.localPosition.y + m_addy,
