...
public class IGuideTask {
    ...
    public enum SelectPlayerType { Newer = 0, Old }
    ...
    public void OnEnter() {
        ...
        IGuideTaskManager.Instance().SendTaskStart((EGameEvent)taskData.EndTaskEvent,
                                                   (EGameEvent)taskData.StartTaskEvent);
    }
    ...
    void OnTrigger(CEvent eve) {
        ...
        switch (tag) {
            ...
            case TaskState.TaskMark:
                ...
                // objList = (ConnectMsg.Instance != null && ConnectMsg.Instance.gameObject.activeInHierarchy) ? null :
                // objList;
                if (taskData.IsMask) {
    ...
    void SkipNewsGuideMark(List<GameObject> objList) {
        if (taskData.TaskId == newsGuideTaskId) {
            // senln
            objSkipNewsGuide =
                LoadUiResource.LoadRes(/*LobbyWindow.Instance.GetRoot()*/ null, GameConstDefine.SkipNewsGuidePath);
