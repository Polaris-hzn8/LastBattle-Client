...
public class LobbyCtrl : Singleton<LobbyCtrl> {
    ...
    public void Exit() { EventCenter.Broadcast(EGameEvent.eGameEvent_LobbyExit); }

    public void AskNewGMCmd(string cmd) { CGLCtrl_GameLogic.Instance.EmsgTocs_AddNewGMCmd(cmd); }

    /// <summary>
    /// 进入大厅的时候初始化引导的信息
    /// </summary>
    private void InitLobbyGuideUIInfo() { UIGuideCtrl.Instance.InitLobbyGuideInfo(); }

    internal void AskPersonInfo() { CGLCtrl_GameLogic.Instance.PersonInfo(); }
