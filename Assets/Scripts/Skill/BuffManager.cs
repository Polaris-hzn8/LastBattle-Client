...
class BuffManager {
    ...
    public bool isHaveStopBuff(UInt64 key) {
        foreach (Buff b in buffDict.Values) {
            ...
            if (bi.effectID == (int)eBuffEffect.eBuffEffectXuanYun ||
                bi.effectID == (int)eBuffEffect.eBuffEffectShuFu) {
                return true;
            }
        }
        return false;
    ...
    public bool isSelfHaveBuffType(int typeID) {
        foreach (Buff b in buffDict.Values) {
            ...
            if (b.buffTypeID == typeID && PlayerManager.Instance.LocalPlayer.ObType == ObPlayerOrPlayer.PlayerType &&
                b.entity.GameObjGUID == PlayerManager.Instance.LocalPlayer.GameObjGUID) {
                return true;
            }
        }
        return false;
    ...
    //
    public bool IsHaveBuff(uint instID) { return buffDict.ContainsKey(instID); }
    ...
    public void RemoveBuff(uint instID) {
        if (buffDict.ContainsKey(instID)) {
            // if (buffDict[instID].buffTypeID == 1017 && buffDict[instID].entity.GameObjGUID ==
            // PlayerManager.Instance.LocalPlayer.GameObjGUID)
            //{
