...
#region 获得所有英雄信息
    ...
    void SortByBuyed() {
        for (int i = 0; i < heroInfoList.Count; i++) {
            for (int j = 0; j < heroInfoList.Count - 1 - i; j++) {
                ...
                if (!GameUserModel.Instance.CanChooseHeroList.Contains(infoA.HeroSelectNum) &&
                    GameUserModel.Instance.CanChooseHeroList.Contains(infoB.HeroSelectNum)) {
    ...
    private void EnterRandomHero() {
        ...
        ResourceUnit clipUnit =
            ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_HERO_SELECT_COUNTDOWN, ResourceType.ASSET);
    ...
    public HeroCtrl() { SetSelectState(HeroSelectState.InitSelect); }

#region public
    // 临时选择的英雄
    public Dictionary<Iplayer, int> heroSelectDic { get; private set; }

    public Dictionary<Iplayer, int> heroSureSelectDic { get; private set; }
    // 所有英雄信息
    public List<HeroSelectConfigInfo> heroInfoList { get; private set; }

    public enum HeroSelectState { InitSelect, EnterSelect, RandomSelect, OutSelect }
#endregion
