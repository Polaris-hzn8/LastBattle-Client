...
public class GamePlayWindow : BaseWindow {
    ...
    protected override void InitWidget() {
        ...
        spritePing[(int)PingEnum.GreenTag] =
            mRoot.Find("UI_AvatarInfos/Adjust/Avatar/PingIcon/GreenIcon").GetComponent<UISprite>();
        spritePing[(int)PingEnum.YellowTag] =
            mRoot.Find("UI_AvatarInfos/Adjust/Avatar/PingIcon/YellowIcon").GetComponent<UISprite>();
        spritePing[(int)PingEnum.RedTag] =
            mRoot.Find("UI_AvatarInfos/Adjust/Avatar/PingIcon/RedIcon").GetComponent<UISprite>();
    ...
    private void ShopBtn(GameObject go) { GamePlayCtrl.Instance.OpenShop(); }
    ...
    private void BattlePlay(GameObject go) { GamePlayCtrl.Instance.AskBattleInfo(); }
    ...
    void ShowDes() {
        if (statePress != DateTime.Now) {
            if (desObj == null) {
                ...
                ResourceUnit desObjUnit = ResourcesManager.Instance.loadImmediate(
                    GameDefine.GameConstDefine.SkillDestribe, ResourceType.PREFAB);
    ...
    public void ShowHomeBaseBeAtkEffect() {
        ...
        ResourceUnit objHomeBaseUnit =
            ResourcesManager.Instance.loadImmediate(GameConstDefine.HomeBaseBeAtkEffect, ResourceType.PREFAB);
    ...
    private void UpdateLockHead(Ientity entity) {
        ...
        if (entity == null) {
            ...
            if (entity.entityType == EntityType.Player) {
                ...
                if (entity.entityType == EntityType.Monster || entity.entityType == EntityType.Soldier ||
                    ...
                    if (player != null && player.AbsorbMonsterType[0] != entity.NpcGUIDType &&
                        player.AbsorbMonsterType[1] != entity.NpcGUIDType) {
                        if (entity.NPCCateChild == ENPCCateChild.eNPCChild_SmallMonster ||
                            entity.NPCCateChild == ENPCCateChild.eNPCChild_HugeMonster ||
                            entity.entityType == EntityType.AltarSoldier &&
                                !PlayerManager.Instance.IsLocalSameType(entity)) {
            ...
            ShowLockEffect(true);
            EventCenter.Broadcast<GameObject>((mCanAbsorb) ? EGameEvent.eGameEvent_GuideLockTargetCanAbsorb
                                                           : EGameEvent.eGameEvent_GuideLockTargetCanNotAbsorb,
                                              spriteLockBg2.gameObject);
        }
    }
    // 显示
    ...
    private void LoadRunFuryEffect() {
        if (null == furyRunEffect) {
            ...
            furyRunEffect.transform.localPosition = new Vector3(sliderFury.transform.localPosition.x,
                                                                sliderFury.transform.localPosition.y,
                                                                sliderFury.transform.localPosition.z - 10f);
            runSlowDown = true;
            startTime = Time.time;
        }
    }
    ...
    private void LoadFullFuryEffect() {
        if (null == furyFullEffect) {
            furyFullEffect =
                LoadUiResource.LoadRes(btnFury.transform, GameDefine.GameConstDefine.FuryFullFuryInfoEffect);
            furyFullEffect.transform.localPosition = new Vector3(sliderFury.transform.localPosition.x,
                                                                 sliderFury.transform.localPosition.y,
                                                                 sliderFury.transform.localPosition.z - 10f);
            startTime = Time.time;
        }
