...
public class LoginWindow : BaseWindow {
    ...
    protected override void OnAddListener() {
        ...
        EventCenter.AddListener<string, string>(EGameEvent.eGameEvent_SdkRegisterSuccess,
                                                SdkRegisterSuccess);  // sdk register success
        EventCenter.AddListener(EGameEvent.eGameEvent_SdkServerCheckSuccess,
                                SdkServerCheckSuccess);                             // sdk register success
    ...
    void OnPlayServer(GameObject go) { ShowServer(LOGINUI.SelectServer); }

    void OnChangeAccount(GameObject go) { LoginCtrl.Instance.SdkLogOff(); }
    ...
    // 登陆成功
    void LoginSuceess() { UIEventListener.Get(mPlaySubmitBtn.gameObject).onClick -= OnPlaySubmit; }
    ...
    IEnumerator ShakeLabel() {
        mPlayEffect =
            GameMethod.CreateWindow(GameConstDefine.LoadGameLoginEffectPath, new Vector3(-5, -270, 0), mRoot.transform);
