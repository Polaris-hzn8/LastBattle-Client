...
public class PrimaryGuideTask : SceneGuideTaskBase {
    ...
    public override void InitSceneTaskData() { heroVoicDic.Clear(); }

    public override void SetTaskStart(int step) { base.SetTaskStart(step); }
    ...
    private void CheckAddHeroBornVoice() {
        if (heroVoicDic != null && heroVoicDic.Count == 3)
            return;
        IEnumerable<Ientity> Items = GameMethod.GetEnemyItemListByRadius(PlayerManager.Instance.LocalPlayer,
