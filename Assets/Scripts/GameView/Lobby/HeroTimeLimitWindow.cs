...
public class HeroTimeLimitWindow : BaseWindow {
    ...
    // 窗口控件释放
    protected override void RealseWidget() { HeroTimeLimitList.Clear(); }
    ...
    private void OnFinishGuideTask(GuideTaskType type, int taskId) { FreshGridPosition(); }

    private void SdkLogOff() { HeroTimeLimitCtrl.Instance.Exit(); }
    ...
    void InitHeroListCards() {
        ...
        foreach (int hero in mHeroList) {
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadGameMarketHeroTag, ResourceType.PREFAB);
    ...
    // Dictionary<GameObject, HeroCard> HeroListShowDict = new Dictionary<GameObject,
    // HeroCard>();//根据类型显示的英雄列表
    ...
    public class HeroCard {
        ...
        public void ReFreshTimeCardTag() {
            ...
            if (conInfo.Consume != null && conInfo.Consume.Count > 1) {
                mGold_Common.transform.Find("LabelMoney1").GetComponent<UILabel>().text =
                    conInfo.Consume.ElementAt(0).Value.ToString();
                mDiamond_Common.transform.Find("LabelMoney1").GetComponent<UILabel>().text =
                    conInfo.Consume.ElementAt(1).Value.ToString();
