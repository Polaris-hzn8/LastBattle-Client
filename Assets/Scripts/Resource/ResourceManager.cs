...
public class ResourceManager {
    ...
    // 获取LoadImplement的数量
    public int getLoadCount() { return loadImplDic.Count; }
    //
    public void Update() {
        if (loadProcess != null) {
        ...
        foreach (LoadImplement l in loadImplDic.Values) {
            if (l.www.isDone == true) {
                if (l.www.error == null) {
                    ...
                    GameDefine.GameMethod.DebugError("Load Res Failed:" + l.www.url + "======" + l.resPath +
                                                     " error is:" + l.www.error.ToString());
                    removeLoadList.Add(l.resPath);
                }
            }
        }
    ...
    public void AsynLoadLogicRes(string path, LogicResouce.ELogicResType type, OnResLoaded onLoaded = null) {
        ...
        switch (type) {
            case LogicResouce.ELogicResType.ERes_Effect: {
                LoadImpl(path, PhyResouce.EPhyResType.EPhyResPrefab, (resPath) => {
    ...
    public void LoadImpl(string path,
                         PhyResouce.EPhyResType type,
                         OnResLoaded onLoaded = null,
                         OnResLoadError onError = null) {
