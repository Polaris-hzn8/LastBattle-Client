...
public class RoomInviteWindow : BaseWindow {
    ...
    private void CloseButton(GameObject go) { RoomCtrl.Instance.ExitInviteList(); }
        ...
        foreach (var item in FriendManager.Instance.RoomInvite) {
            ...
            if (!OnLineNickName.TryGetValue(item.Key, out olf)) {
                ResourceUnit objUnit =
                    ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadInvitationUI, ResourceType.PREFAB);
    ...
    // 显示
    public override void OnEnable() { ShowFriendOnLine(); }
