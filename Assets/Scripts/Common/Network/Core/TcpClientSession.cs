...
public abstract class TcpClientSession : ClientSession {
    ...
    public TcpClientSession(EndPoint remoteEndPoint) : this(remoteEndPoint, 1024) {}

    public TcpClientSession(EndPoint remoteEndPoint, int receiveBufferSize) : base(remoteEndPoint) {
...
// WindowsPhone doesn't have this property
    ...
    protected void ProcessConnect(Socket socket, object state, SocketAsyncEventArgs e) {
        ...
        // one user reported in some cases the e.SocketError = SocketError.Succes but the socket is not connected in
        // MonoTouch
        if (!socket.Connected) {
            m_InConnecting = false;
    ...
    protected bool EnsureSocketClosed() { return EnsureSocketClosed(null); }
    ...
    protected bool EnsureSocketClosed(Socket prevClient) {
        lock (this) {
            ...
            if (prevClient != null &&
                prevClient !=
                    client)  // originalClient is previous disconnected socket, so we needn't fire event for it
            {
                client = prevClient;
    ...
    protected void OnSendingCompleted() { m_IsSending = 0; }
