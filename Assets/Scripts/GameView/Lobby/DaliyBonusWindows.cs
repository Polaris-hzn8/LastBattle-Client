...
public class DaliyBonusWindow : BaseWindow {
    ...
    private void OnCloseBtnClick() { DailyBonusCtrl.Instance.Exit(); }
    ...
    private void OnDaliyGameObjsClick(GameObject obj) {
        ...
        DailyBonusConfigInfo bonus =
            ConfigReader.GetDailyBonusInfo(DailyBonusCtrl.Instance.mHadDay + 1);  // 设置奖励坑位（1.金币 2.钱 3.物品）
        if (bonus == null)
            return;  // 没有奖励不显示
        ...
        foreach (string itemStr in bonus.n32ItemID) {
            ...
            if (itemID == 1) {
                EventCenter.Broadcast<EPurchaseType, string, string, int>(
                    EGameEvent.eGameEvent_PurchaseRuneSuccessWin, EPurchaseType.EPT_Gold, "金币", "", count);
            } else if (itemID == 2) {
                EventCenter.Broadcast<EPurchaseType, string, string, int>(
                    EGameEvent.eGameEvent_PurchaseRuneSuccessWin, EPurchaseType.EPT_Crystal, "钻石", "", count);
            } else if (itemID > 100000 && itemID < 110000) {
                EventCenter.Broadcast<EPurchaseType, string, string, int>(
                    EGameEvent.eGameEvent_PurchaseRuneSuccessWin,
                    EPurchaseType.EPT_Hero,
                    ConfigReader.HeroBuyXmlInfoDict[(int)itemID].Name,
                    ConfigReader.HeroBuyXmlInfoDict[(int)itemID].DefaultIcon,
                    count);
            } else if (itemID > 110000 && itemID < 120000) {
            } else if (itemID > 120000 && itemID < 130000) {
                EventCenter.Broadcast<EPurchaseType, string, string, int>(
                    EGameEvent.eGameEvent_PurchaseRuneSuccessWin, EPurchaseType.EPT_Rune, "符文AA", "", count);
            } else if (itemID > 130000 && itemID < 140000) {
                EventCenter.Broadcast<EPurchaseType, string, string, int>(
                    EGameEvent.eGameEvent_PurchaseRuneSuccessWin,
                    EPurchaseType.EPT_TrialCard,
                    ConfigReader.OtherItemXmlInfoDic[itemID].sName,
                    "",
                    count);
            }
        }
    ...
    private void OnEvent_DailyBonusUpdate() {
        ResourceUnit GetMoneyUnit =
            ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadDailyHeroIcon, ResourceType.PREFAB);
        ...
        for (uint i = 0; i < DailyBonusCtrl.Instance.mMaxDay; ++i) {
            ...
            //////////////////////////////////////////////////////////////////////////
            if (i < DailyBonusCtrl.Instance.mHadDay) {  // 已领取
                ...
                if (DailyBonusCtrl.Instance.mTodayCan) {  // 可领取
            ...
            foreach (string itemStr in bonus.n32ItemID) {
                ...
                GameObject oneBouns =
                    LoadUiResource.AddChildObject(tmpGrid.transform, GameConstDefine.LoadDailyOneBonusUI);
                ...
                if (itemID == 1) {
                    ...
                    oneBouns.transform.Find("Name").GetComponent<UILabel>().text =
                        ConfigReader.HeroBuyXmlInfoDict[(int)itemID].Name;
                    oneBouns.transform.GetComponent<UISprite>().spriteName =
                        ConfigReader.HeroBuyXmlInfoDict[(int)itemID].DefaultIcon;
                } else if (itemID > 110000 && itemID < 120000) {
                    oneBouns.transform.Find("Name").GetComponent<UILabel>().text = "皮肤";
                    oneBouns.transform.GetComponent<UISprite>().spriteName =
                        ConfigReader.HeroSkinXmlInfoDict[(int)itemID].Icon;
                } else if (itemID > 120000 && itemID < 130000) {
                    ...
                    oneBouns.transform.Find("Name").GetComponent<UILabel>().text =
                        ConfigReader.OtherItemXmlInfoDic[itemID].sName;
                    oneBouns.transform.GetComponent<UISprite>().spriteName =
                        ConfigReader.OtherItemXmlInfoDic[itemID].icon;
                } else {
                    oneBouns.transform.Find("Name").GetComponent<UILabel>().text = "????";
    ...
    private void OnEvent_DailyTaskUpdate() {
        ...
        foreach (KeyValuePair<uint, CTask> one in DailyBonusCtrl.Instance.mDailyTaskDic) {
            GameObject addobj =
                LoadUiResource.AddChildObject(mDailyTaskGrid.transform, GameConstDefine.LoadDailyTaskUI);
    ...
    private void OnEvent_InfiniteTaskUpdate() {
        ...
        foreach (KeyValuePair<uint, CTask> one in DailyBonusCtrl.Instance.mInfiniteTaskDic) {
            GameObject addobj =
                LoadUiResource.AddChildObject(mInfiniteTaskGrid.transform, GameConstDefine.LoadInfiniteTaskUI);
            addobj.name = Convert.ToString(one.Value.mGuid);
            addobj.transform.Find("TaskName").GetComponent<UILabel>().text = one.Value.mConfig.taskDesc;
            UpdateTaskToUI(addobj, one.Value);
        }
    ...
    private void OnTaskFinishBtnClick(GameObject obj) {
        Debug.Log("DaliyGameObjs" + obj.name + "clicked");
        GCToCS.AskOneTaskRewards pMsg = new GCToCS.AskOneTaskRewards() { task_guid = Convert.ToUInt32(obj.name) };
    ...
    private void UpdateTaskToUI(GameObject uiTask, CTask task) {
        ...
        if (task.mCurCount == UInt32.MaxValue) {  // 已领取
        ...
        } else if (task.mCurCount == task.mConfig.taskMaxCount) {  // 已完成
        ...
        uiTask.transform.Find("ProgressLabel").GetComponent<UILabel>().text =
            Convert.ToString(curCount) + "/" + Convert.ToString(task.mConfig.taskMaxCount);
    }
}
}
