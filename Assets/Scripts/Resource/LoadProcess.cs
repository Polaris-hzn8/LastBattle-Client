...
public class LoadProcess {
    ...
    public int curLoadCount {
        get { return ResourceManager.Instance.getLoadCount(); }
    }
    ...
    // 完成的文件个数
    public int doneFileCount { get; set; }
    // 总的文件物理大小
    public int filesSize { get; set; }
    // 总的完成文件物理大小
    public int doneFilesSize { get; set; }
                ...
                if (curLoadCount < aSyncLoadCount) {
                    if (ResourceManager.Instance.GetLogicRes(logicLoadReq.resPath) != null) {
                        ...
                        if (logicLoadReq.status == LogicLoadRequest.LOADSTATUS.WAIT) {
                            ResourceManager.Instance.LoadImpl(
                                logicLoadReq.resPath, LogicResouce.getRootResType(logicLoadReq.resType), (path) => {
                                    AssetBundle ab = ResourceManager.Instance.GetPhyRes(path).assetBundle;
                                    StringScriptableObject holder =
                                        (StringScriptableObject)ab.LoadAsset("DependentBundleNames");
