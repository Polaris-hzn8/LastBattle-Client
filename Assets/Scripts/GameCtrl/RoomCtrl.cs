...
public class RoomCtrl : Singleton<RoomCtrl> {
    public void Enter() { EventCenter.Broadcast(EGameEvent.eGameEvent_RoomEnter); }

    public void Exit() { EventCenter.Broadcast(EGameEvent.eGameEvent_RoomExit); }

    // 改变座位
    public void ChangeSeat(int i) { CGLCtrl_GameLogic.Instance.ChangeRoomSeat(i); }

    // 开始战场
    public void BeginBattle() { CGLCtrl_GameLogic.Instance.StartRoom(); }

    // 准备
    public void ReadyRoom() { CGLCtrl_GameLogic.Instance.ReadyRoom(); }

    // 取消准备
    public void CancelReadyRoom() { CGLCtrl_GameLogic.Instance.CancelRoom(); }
    ...
    public void ExitInviteList() { EventCenter.Broadcast(EGameEvent.eGameEvent_RoomInviteExit); }
