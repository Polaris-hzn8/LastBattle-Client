...
public class UIPvpOrGuideBase : MonoBehaviour {
    ...
    public static UIPvpOrGuideBase Instance { set; get; }
    ...
    void OnEnable() {
        ...
        MoveSprite.count = 0;
        // UIGuide
        IGuideTaskManager.Instance().AddTaskStartListerner(EGameEvent.eGameEvent_UIGuideNewsGuideStart,
                                                           StartIGuideTask);
        IGuideTaskManager.Instance().AddTaskStartListerner(EGameEvent.eGameEvent_UIGuideSelfDefBtnStart,
                                                           StartIGuideTask);
    ...
    void Update() {
        ...
        if (ChangeDate.fillAmount < 1 && ChangeDate.fillAmount >= 0) {
            ...
            MatteBg.transform.localPosition =
                new Vector3(MatteBg.transform.position.x - 1000, -y, MatteBg.transform.position.z);
        }
    ...
    void OnDisable() {
        IGuideTaskManager.Instance().RemoveTaskStartListerner(EGameEvent.eGameEvent_UIGuideNewsGuideStart,
                                                              StartIGuideTask);
        IGuideTaskManager.Instance().RemoveTaskStartListerner(EGameEvent.eGameEvent_UIGuideSelfDefBtnStart,
                                                              StartIGuideTask);
        IGuideTaskManager.Instance().RemoveTaskStartListerner(EGameEvent.eGameEvent_UIGuideMatchBtnStart,
                                                              StartIGuideTask);
