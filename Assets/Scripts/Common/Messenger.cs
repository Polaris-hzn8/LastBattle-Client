...
//	static private MessengerHelper mMessengerHelper = ( new GameObject("MessengerHelper") ).AddComponent<
//MessengerHelper >();
#pragma warning restore 0414
    ...
    static public void OnListenerAdding(EGameEvent eventType, Delegate listenerBeingAdded) {
        ...
        Debug.Log("MESSENGER OnListenerAdding \t\"" + eventType + "\"\t{" + listenerBeingAdded.Target + " -> " +
        ...
        if (d != null && d.GetType() != listenerBeingAdded.GetType()) {
            throw new ListenerException(
                string.Format("Attempting to add listener with inconsistent signature for event type {0}. Current " +
                              "listeners have type {1} and listener being added has type {2}",
                              eventType,
                              d.GetType().Name,
...
#if LOG_ALL_MESSAGES
        Debug.Log("MESSENGER OnListenerRemoving \t\"" + eventType + "\"\t{" + listenerBeingRemoved.Target + " -> " +
...
#if REQUIRE_LISTENER
        if (!mEventTable.ContainsKey(eventType)) {
            // throw new BroadcastException(string.Format("Broadcasting message \"{0}\" but no listener found. Try
            // marking the message with Messenger.MarkAsPermanent.", eventType));
        }
#endif
    }
