...
public class MailCtrl : Singleton<MailCtrl> {
    public void Enter() { EventCenter.Broadcast(EGameEvent.eGameEvent_MailEnter); }

    public void Exit() { EventCenter.Broadcast(EGameEvent.eGameEvent_MailExit); }
    ...
    public void AddMail(GSToGC.NotifyMailList pMsg) {
        foreach (GSToGC.ShortMail mail in pMsg.mailList) {
            MailModel.Instance.AddMailData(
                (int)mail.mailType, mail.mailid, mail.mailTitle, (EMailCurtState)mail.mailState);
        }
        // 从新排序
    ...
    public void GetMailContent(int mailId) {
        ...
        GCToCS.AskMailInfo pMsg = new GCToCS.AskMailInfo() { mailId = mailId };
    ...
    public void CloseOrGetMailGift(int mailId) {
        ...
        GCToCS.AskGetMailGift pMsg = new GCToCS.AskGetMailGift() { mailId = mailId };
        NetworkManager.Instance.SendMsg(pMsg, (int)pMsg.msgnum);
    }
}
}
