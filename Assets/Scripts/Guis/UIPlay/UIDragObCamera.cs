...
public class UIDragObCamera : MonoBehaviour {
    ...
    public void SetUsable(bool can) { canUse = can; }
    ...
    bool InEquipArea(Vector3 pos) {
        if (pos.x >= (Screen.width / 2f - equipXLimit) && pos.x <= (Screen.width / 2f + equipXLimit) && pos.y > 10f &&
            pos.y < 50f)
            return true;
        return false;
    }
    ...
    bool InSkillArea(Vector3 pos) {
        if (pos.x < (Screen.width - ConnerXLimit) && pos.x >= (Screen.width - skillXLimit) && pos.y < 50f &&
            pos.y > 10f)
            return true;
        return false;
    }
