...
public class UIGameOver : MonoBehaviour {
    ...
    public static UIGameOver Instance { private set; get; }
    ...
    public void RemoveList() { ListDate.Clear(); }
    void OnDisble() {
        Instance = null;
    ...
    public void SetScoreInfo(
        UInt64 key, string pszNickName, uint level, uint heroKills, uint deadTimes, EntityCampType Type) {
        isStartTime = true;
    ...
    public void Show() {
        ...
        foreach (var item in ListDate) {
            if ((EntityCampType)item.HeroCampID == EntityCampType.CampTypeA) {
                if (i <= 2)
                    ListPlayer[i++].SetShowLabel(item.HeadIcon,
                                                 item.HeadName,
                                                 item.HeroLevel,
                                                 item.HeroKills,
                                                 item.HeroDeadTimes,
                ...
                if (num <= 5)
                    ListPlayer[num++].SetShowLabel(item.HeadIcon,
                                                   item.HeadName,
                                                   item.HeroLevel,
                                                   item.HeroKills,
                                                   item.HeroDeadTimes,
