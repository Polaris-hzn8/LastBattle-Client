...
public class ScoreCtrl : Singleton<ScoreCtrl> {
    public void Enter() { EventCenter.Broadcast(EGameEvent.eGameEvent_ScoreEnter); }

    public void Exit() { EventCenter.Broadcast(EGameEvent.eGameEvent_ScoreExit); }
    ...
    public void SetScoreInfo(long gotGole, int oldExp, int newExp, short oldLevel, short newLevel, int upLevelExp) {
        ...
        // Debug.LogError(gotGole + " oldExp = " + oldExp + " newExp = " + newExp + "  oldLevel = " + oldLevel + "
        // newLevel = " + newLevel + " upLevelExp = " + upLevelExp);
        GameUserModel.Instance.SetGameUserLv((short)mGotLevel);  // 设置升级后等级和经验
        GameUserModel.Instance.SetGameUserExp(mUpLevelExt);
    }

    // showInfo
    public void SetSettlementInfo(UInt64 objGUID,
                                  int headID,
                                  string nickName,
                                  int kills,
                                  int death,
    ...
    public List<SettlementData> GetSettlementList() { return mListMentDate; }
    ...
    public class SettlementData {
        ...
        public void Init(UInt64 objGUID,
                         int headID,
                         string nickName,
                         int level,
                         int kills,
