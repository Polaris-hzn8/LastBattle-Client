...
public class SkillWindow : BaseWindow {
    ...
    protected override void InitWidget() {
        ...
        // 暴气技能初始化
        ResourceUnit effectUnit =
            ResourcesManager.Instance.loadImmediate(GameDefine.GameConstDefine.FurySkillBtnEffect, ResourceType.PREFAB);
    ...
    public override void OnDisable() {
        EventCenter.Broadcast<bool, SkillType, Iplayer>(
            EGameEvent.eGameEvent_SkillDescribleType, false, SkillType.SKILL_NULL, PlayerManager.Instance.LocalPlayer);
        mIsShowDes = false;
        mCurSkillPress = -1;
        mTimePressStart = 0f;
    }
    ...
    public override void Update(float deltaTime) {
        ...
        if (Time.time - mTimePressStart >= mTimeLimit) {
            EventCenter.Broadcast<bool, SkillType, Iplayer>(EGameEvent.eGameEvent_SkillDescribleType,
                                                            true,
                                                            GetSkillType(mCurSkillPress),
    ...
    bool IsHeroSkillBtn(int index) {
        if (ShortCutBarBtn.BTN_SKILL_1 == (ShortCutBarBtn)index ||
            ...
            ShortCutBarBtn.BTN_SKILL_3 == (ShortCutBarBtn)index || ShortCutBarBtn.BTN_SKILL_4 == (ShortCutBarBtn)index)
            return true;
        return false;
    }
    ...
    private void OnLockEnemySoldier() {
        ...
        if (GamePlayGuideModel.Instance.IsGuideTrigger(ButtonTriggerType.mTypeClick, mGameObjectLock)) {
            itemList = GameMethod.GetMonsterEnemyItemListByRadius(PlayerManager.Instance.LocalPlayer,
                                                                  GameConstDefine.PlayerLockTargetDis);
        } else {
            itemList = GameMethod.GetSoldierEnemyItemListByRadius(PlayerManager.Instance.LocalPlayer,
                                                                  GameConstDefine.PlayerLockTargetDis);
        }
    ...
    public void CreateEffect(Transform tran, string pathName) {
        ResourceUnit objUnit =
            ResourcesManager.Instance.loadImmediate("effect/ui_effect/" + pathName, ResourceType.PREFAB);
    ...
    public void ShowPressEffect(ShortCutBarBtn btnType) {
        ...
        ResourceUnit objUnit =
            ResourcesManager.Instance.loadImmediate(GameConstDefine.PressBtnEffect, ResourceType.PREFAB);
    ...
    public void ShowCdEndEffect(ShortCutBarBtn btnType, bool show) {
        ...
        if (show) {
            ...
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.CdEndEffect, ResourceType.PREFAB);
    ...
    private void ShowBtnCanUseEffect(ShortCutBarBtn type) {
        if (PlayerManager.Instance == null || PlayerManager.Instance.LocalPlayer == null)
            return;
        if (PlayerManager.Instance.LocalPlayer.SkillIdDic == null ||
            PlayerManager.Instance.LocalPlayer.SkillIdDic.Count == 0)
            return;
        Iselfplayer player = PlayerManager.Instance.LocalPlayer;
    ...
    // 判断某个技能是否在CD中
    public bool IsSkillInCd(ShortCutBarBtn type) { return mSkillCdList.Contains(type); }
    ...
    private void OnLocalPlayerInit() {
        ...
        if (ConfigReader.GetSkillManagerCfg(heroInfo.HeroSkillType1) != null) {
            SetSkillBtnPic(ShortCutBarBtn.BTN_SKILL_1,
                           ConfigReader.GetSkillManagerCfg(heroInfo.HeroSkillType1).skillIcon);
        }
        if (ConfigReader.GetSkillManagerCfg(heroInfo.HeroSkillType2) != null) {
            SetSkillBtnPic(ShortCutBarBtn.BTN_SKILL_2,
                           ConfigReader.GetSkillManagerCfg(heroInfo.HeroSkillType2).skillIcon);
        }
        if (ConfigReader.GetSkillManagerCfg(heroInfo.HeroSkillType3) != null) {
            SetSkillBtnPic(ShortCutBarBtn.BTN_SKILL_3,
                           ConfigReader.GetSkillManagerCfg(heroInfo.HeroSkillType3).skillIcon);
        }
        if (ConfigReader.GetSkillManagerCfg(heroInfo.HeroSkillType4) != null) {
            SetSkillBtnPic(ShortCutBarBtn.BTN_SKILL_4,
                           ConfigReader.GetSkillManagerCfg(heroInfo.HeroSkillType4).skillIcon);
        }
    ...
    private Dictionary<ShortCutBarBtn, GameObject> mCanPressEffect =
        new Dictionary<ShortCutBarBtn, GameObject>();  // 可点击特效列表
    private Dictionary<ShortCutBarBtn, UISprite> mBtnSprite =
        new Dictionary<ShortCutBarBtn, UISprite>();  // 技能按钮图片列表
    private Dictionary<ShortCutBarBtn, ButtonSelectPic> mBtnSelectPic =
        new Dictionary<ShortCutBarBtn, ButtonSelectPic>();  // 技能选择图片列表
