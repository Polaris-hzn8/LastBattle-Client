...
public class ShakeCamera : MonoBehaviour {
    ...
    public void ShakeScreen(
        ShakeOrient shakeOrient, float period, float shakeTime, float maxWave, float minWave, float offPeriod = 0) {
        // 不在振动状态
        if (!mIsShake) {
            // 确保Transform有效
        ...
        if (mIsShake) {
            ...
            if (mShakeOrient == ShakeOrient.vertical)
                transform.localPosition =
                    new Vector3(transform.localPosition.x, mDefaultPos.y, transform.localPosition.z) + mShakeDir* value;
            else
