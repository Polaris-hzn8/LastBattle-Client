...
public class Iselfplayer : Iplayer {
    public Iselfplayer(UInt64 sGUID, EntityCampType campType) : base(sGUID, campType) {
    ...
    public Dictionary<int, int> UserGameItems { private set; get; }

    public Dictionary<int, int> UserGameItemsCount { private set; get; }

    public Dictionary<int, float> UserGameItemsCoolDown { private set; get; }

    public int[] AbsorbMonsterType = new int[2];

    public GameObject objWarning { private set; get; }
    ...
    public override void OnUpdate() {
        ...
        if (SyncLockTarget != null && SyncLockTarget.realObject != null) {
            if (Vector3.Distance(this.objTransform.position, SyncLockTarget.objTransform.position) >
                GameConstDefine.PlayerLockTargetDis) {
                if (!PlayerManager.Instance.IsLocalSameType(SyncLockTarget) &&
                        SyncLockTarget.entityType == EntityType.Monster ||
                    SyncLockTarget.entityType == EntityType.Soldier) {
                    if (SyncLockTarget.XueTiao != null) {
                        SyncLockTarget.XueTiao.IsXueTiaoCpVib(false);
                    }
                }
        ...
        if (AutoLockTarget) {
            IEnumerable<Ientity> ItemList = GameMethod.GetEnemyItemListByRadius(PlayerManager.Instance.LocalPlayer,
                                                                                GameConstDefine.PlayerLockTargetDis);
    ...
    public override void OnEnterEntityAdMove() { base.OnEnterEntityAdMove(); }
    ...
    public override void OnExecuteEntityAdMove() {
        ...
        Quaternion sMidQuater =
            Quaternion.Lerp(RealEntity.GetTransform().rotation, DestQuaternion, 10 * Time.deltaTime);
    ...
    public void initAreaCircle() {
        ...
        if (CircleArea == null) {
            ...
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(path + "guangquan_fanwei", ResourceType.PREFAB);
    ...
    private void UpdataRange() {
        ...
        if (RealEntity == null)
            return;
        CircleArea.transform.localScale = new Vector3(this.AtkDis * 2.0f / RealEntity.GetTransform().localScale.x,
                                                      1.0f,
                                                      this.AtkDis * 2.0f / RealEntity.GetTransform().localScale.z);
    }
    //
    ...
    private void checkCircleRes() {
        ...
        if (CircleGreen == null) {
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(path + "guangquan_green", ResourceType.PREFAB);
            CircleGreen = GameObject.Instantiate(objUnit.Asset) as GameObject;
        }
        if (CircleYellow == null) {
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(path + "guangquan_yellow", ResourceType.PREFAB);
    ...
    //
    public override void OnAttackAnimationEnd() { OnFSMStateChange(EntityFreeFSM.Instance); }
    // 请求吸附怪物
    ...
    public void SendAbsortMonster() {
        ...
        if (SyncLockTarget.entityType == EntityType.Building || SyncLockTarget.entityType == EntityType.Player ||
            SyncLockTarget.entityType == EntityType.Soldier) {
        ...
        if (IsAbsobing == true)
            return;
        if (AbsorbMonsterType[0] != 0 && AbsorbMonsterType[1] != 0 && Level >= absBigLevel &&
            (AbsorbMonsterType[1] != SyncLockTarget.NpcGUIDType &&
             AbsorbMonsterType[0] != SyncLockTarget.NpcGUIDType) &&
            isLock != 0) {
            CreateSoleSoldier();
            return;
        }
        if (AbsorbMonsterType[0] != 0 && Level >= absSmallLevel && AbsorbMonsterType[0] != SyncLockTarget.NpcGUIDType &&
            isLock != 0 && AbsorbMonsterType[1] == 0 && AbsorbMonsterType[1] != SyncLockTarget.NpcGUIDType &&
            Level < absBigLevel) {
            CreateSoleSoldier();
            return;
        }
    ...
    public override int GetSkillIdBySkillType(SkillType type) { return GetSkillId(type); }

    public override void OnAttackEnd() { RealEntity.PlayerFreeAnimation(); }
    ...
    private void CleanSkillDic(Dictionary<SkillType, int> skillDic) { skillDic.Clear(); }
    ...
    public override void OnSkillInfoChange(int skillID, float time, float maxTime, int slot) {
        ...
        EventCenter.Broadcast<SkillType, float, float>(
            EGameEvent.eGameEvent_LocalPlayerSkillCD, skillType, maxTime, time);
    }
    // 获取对应技能的冷却时间
    ...
    public override void InitWhenCreateModel() {
        ...
        ResourceUnit WalkAudioClipUnit = ResourcesManager.Instance.loadImmediate(
            AudioDefine.PATH_HERO_WALKSOUND + ConfigReader.GetHeroInfo(this.NpcGUIDType).un32WalkSound,
            ResourceType.ASSET);
        WalkAudioClip = WalkAudioClipUnit.Asset as AudioClip;

        ResourceUnit DeadAudioClipUnit = ResourcesManager.Instance.loadImmediate(
            AudioDefine.PATH_HERO_DEADSOUND + ConfigReader.GetHeroInfo(this.NpcGUIDType).HeroDeathSould,
            ResourceType.ASSET);
        DeadAudioClip = DeadAudioClipUnit.Asset as AudioClip;

        ResourceUnit EnterAltarClipUnit =
            ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_ENTERALTAR_SOUND, ResourceType.ASSET);
        EnterAltarClip = EnterAltarClipUnit.Asset as AudioClip;

        ResourceUnit RebornClipUnit =
            ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_REBORN_SOUND, ResourceType.ASSET);
    ...
    public AudioClip GetAltarClip() { return EnterAltarClip; }
    ...
    public void AddRuinWarning() {
        ...
        ResourceUnit objWarningUnit =
            ResourcesManager.Instance.loadImmediate(GameDefine.GameConstDefine.WarningRuin, ResourceType.PREFAB);
