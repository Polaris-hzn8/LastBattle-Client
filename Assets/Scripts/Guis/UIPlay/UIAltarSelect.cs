...
public class UIAltarSelect : MonoBehaviour {
    public static UIAltarSelect Instance { private set; get; }
    ...
    /// #############################
    /// </summary>
    public Ientity TrAltar { private set; get; }

    void OnEnable() {
        Instance = this;
        SoldierSelectBtn = new List<ButtonOnPress>();
        AltarHeadPhoto = new List<UISprite>();
        ButtonOnPress mBtnSoldier1 =
            this.transform.Find("HeadShow/SoldierSelect/Soldier1").GetComponent<ButtonOnPress>();
    ...
    private void ShowAltarSelectPic() {
        ...
        for (int i = 0; i < 2; i++) {
            if (pl.AbsorbMonsterType[i] != 0) {
                ...
                AltarHeadPhoto[1 + i].spriteName =
                    ConfigReader.GetNpcInfo((int)pl.AbsorbMonsterType[i]).HeadPhoto.ToString();
            }
        }
        if (TrAltar.EntityHoldNpcType == 0) {
            if (AltarData.AllAltarDic != null && AltarData.AllAltarDic.Count != 0 &&
                AltarData.AllAltarDic.ContainsKey(TrAltar.MapObgId)) {
    ...
    private void OnSoldierSelectFunc(int ie, bool presses) {
        ...
        if (TrAltar.EntityHoldNpcType != 0 && TrAltar != null) {
            ...
            // Debug.LogError(npcType + "   TrAltar = " + TrAltar + " AltarManager.Instance = " +
            // AltarManager.Instance);
