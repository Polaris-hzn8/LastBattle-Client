...
class CTools {
    ...
    public static EObjectType GetGUIDType(UInt64 sGUID) { return (EObjectType)(sGUID >> 48); }
    ...
    public static Boolean IfTypeNPC(EObjectType eOT) {
        if ((Int32)EObjectType.eObjectType_NPCBegin < (Int32)eOT &&
            (Int32)eOT < (Int32)EObjectType.eObjectType_NPCBegin + (Int32)EConstEnum.eObjLevel1Inter) {
            return true;
        }
        return false;
    }
    ...
    public static Boolean IfTypeHero(EObjectType eOT) {
        if ((Int32)EObjectType.eObjectType_HeroBegin < (Int32)eOT &&
            (Int32)eOT < (Int32)EObjectType.eObjectType_HeroBegin + (Int32)EConstEnum.eObjLevel1Inter) {
            return true;
        }
        return false;
    }
    ...
    public static bool CheckName(string text) {
        ...
        for (int i = 0; i < text.Length; i++) {
            ch = text[i];
            if (!((ch >= 0x4e00 && ch <= 0x9fbb) || (ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z') ||
                  (ch >= '0' && ch <= '9'))) {
                return false;
            }
        }
