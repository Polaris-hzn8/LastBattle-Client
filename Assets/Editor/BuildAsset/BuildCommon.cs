...
public class BuildCommon {
    ...
    // public static void getNextLevelDependencedAllAssetSize(AssetUnit asset, List<string> LeveldependencedPrefabs, ref
    // int count)
    //{
    //     Object mainAsset = AssetDatabase.LoadMainAssetAtPath(asset.mPath);
    //     bool isBatchingStatic = ((asset.mSuffix == "prefab") &&
    //         (StaticEditorFlags.BatchingStatic == (StaticEditorFlags.BatchingStatic &
    //         GameObjectUtility.GetStaticEditorFlags(mainAsset as GameObject))));
    ...
    //        //剔除静态批次prefab依赖的fbx
    //        if (isBatchingStatic && nextdeps.mPath.ToLower().Contains(".fbx") &&
    //        !nextdeps.mPath.ToLower().Contains("_coll"))
    //        {
    ...
    // public static void writePrefabDependence(ref AssetExtraInfo extraInfo, List<string> LeveldependencedPrefabs,
    // AssetUnit prefab)
    //{
    //     foreach(AssetUnit asset in prefab.mNextLevelDependencies)
    //     {
