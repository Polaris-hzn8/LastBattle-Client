...
public class UIGuideOver : MonoBehaviour {
    ...
    void PressOver(int ie, bool Press) { NotifyGameOver(); }
    ...
    void Update() {
        if (timeLimit > 0f) {
            ...
            if ((timeBetween > timeLimit) && !hasShowEffect) {
                ...
                hasShowEffect = true;
            } else if ((timeBetween > timeLimit + 0.5f) && timeBetween < (timeLimit + showTime + 0.5f) &&
                       (!objUI.activeInHierarchy)) {
    ...
    public static void LoadGuideOver() {
        ...
        ResourceUnit objUnit =
            ResourcesManager.Instance.loadImmediate(GameConstDefine.GuideOverUi, ResourceType.PREFAB);
        ...
        if (SceneGuideTaskManager.Instance().sceneGuideTask != null &&
            SceneGuideTaskManager.Instance().sceneGuideTask.currentTask != null) {
