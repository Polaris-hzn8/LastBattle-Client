...
#region 获得所有英雄信息
    ...
    private void CloseHeadBtn(GameObject go) { mTempHeadIcon.gameObject.SetActive(false); }
    ...
    private void ClosePresonInfo(GameObject go) { PresonInfoCtrl.Instance.Exit(); }
    // 显示VIP特权介绍
    private void ShowVipPriv(GameObject go) { VIPPrerogativeCtrl.Instance.Enter(); }
    ...
    private void SureClickBtn(GameObject go) {
        if (string.Compare(mNewTemp.value.Trim(), "请输入要更换的昵称") == 0 ||
            string.Compare(mNewTemp.value.Trim(), "输入新的昵称") == 0)
            return;
...
#region 加载选择英雄显示预设
                ...
                ResourceUnit objUnit =
                    ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadHeadPhotoUi, ResourceType.PREFAB);
    ...
    private void ChangeNickName() { mNickName.text = GameUserModel.Instance.GameUserNick; }

    private void ChangeGoldTime() {
        if (PresonInfoCtrl.Instance.mGoldHours != 0 || PresonInfoCtrl.Instance.mGoldMinute != 0 ||
            PresonInfoCtrl.Instance.mGoldSecond != 0) {
    ...
    private void ChangeExpTime() {
        if (PresonInfoCtrl.Instance.mExpHours != 0 || PresonInfoCtrl.Instance.mExpMinute != 0 ||
            PresonInfoCtrl.Instance.mExpSecond != 0) {
    ...
    public override void OnEnable() {
        ...
        if (leveinfo != null) {
            ...
            mExp.transform.parent.GetComponent<UISprite>().fillAmount =
                PresonInfoCtrl.Instance.mExp / leveinfo.LevelUpExp;
        }
        if (level >= 30) {
