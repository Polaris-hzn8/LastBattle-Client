...
public static class GameMethod {
    ...
    public static GameObject LoadProgress() {
        // DisableAllUI ();
        // GameObject LoadPregress =
        // (GameObject)UnityEngine.Object.Instantiate(Resources.Load(GameConstDefine.LoadProsPath));

        ResourceUnit LoadPregressUnit =
            ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadProsPath, ResourceType.PREFAB);
    ...
    public static void RemoveUI(string name) {
        ...
        for (int i = 0; i < GetUiCamera.transform.childCount; i++) {
            //				MonoBehaviour.DestroyImmediate()
            if (GetUiCamera.transform.GetChild(i) != null && GetUiCamera.transform.GetChild(i).gameObject != null &&
                GetUiCamera.transform.GetChild(i).gameObject.name == name) {
                GameObject.DestroyImmediate(GetUiCamera.transform.GetChild(i).gameObject);
            }
        }
    }
    ...
    public static IEnumerable<Ientity> GetEnemyItemListByRadius(Ientity self, float radius) {
        var items = from item in EntityManager.AllEntitys
                        .Values
                    where item.RealEntity !=
                    null
                    where item.RealEntity.enabled ==
                    true
                    where item.entityType !=
                    EntityType.Monster
                    where IsEnemyCamp(self,
  item)
                                              where item.FSM !=
  null
                                      where item.FSM.State !=
  BlGame.FSM.FsmState.FSM_STATE_DEAD
                                      where item.mCanBeLocked let distance =
  Vector3.Distance(item.realObject.transform.position, self.realObject.transform.position)
                                                           where distance<radius orderby distance select item;
        return items;
    }
  ...
  BlGame.FSM.FsmState.FSM_STATE_DEAD
                                      where item.entityType !=
  EntityType.Monster
                                      where item.entityType != EntityType.Building let distance =
  Vector3.Distance(item.realObject.transform.position, self.realObject.transform.position)
                                                           where distance<radius orderby distance select item;
        return items.ToList();
    }
  ...
  EntityType.Building
                                      where item.entityType !=
  EntityType.Monster
                                      where item.entityType != EntityType.Player let distance =
  Vector3.Distance(item.realObject.transform.position, self.realObject.transform.position)
                                                           where distance<radius orderby distance select item;
        return items.ToList();
    }
  ...
  Vector3.Distance(item.realObject.transform.position, self.realObject.transform.position)
    ...
    public static void BuildReplaceObject(Ientity entity) {
        ...
        //             ResourcesManager.Instance.loadImmediate(GameConstDefine.MapObjectReplaceIDPath + rp,
        //             ResourceType.PREFAB); GameObject obj = objUnit.Asset as GameObject;
        //
        //             if (obj == null)
        //             {
        //                 return;
        //             }
        // root = GameObject.Instantiate(obj , entity.realObject.transform.position , Quaternion.identity) as
        // GameObject;
    ...
    public static Vector3 ResolveToVector3AsServer(string values, char sp = ',') {
        ...
        return new Vector3(
            Convert.ToSingle(value[0]) / 100f, Convert.ToSingle(value[1]) / 100f, Convert.ToSingle(value[2]) / 100f);
    }
    ...
    public static Color ResolveToColor(string values) {
        ...
        return new Color(
            Convert.ToSingle(value[0]) / 255f, Convert.ToSingle(value[1]) / 255f, Convert.ToSingle(value[2]) / 255f);
    }
    ...
    public static void SetLoadJianTaBaoZha(Ientity entity, bool Replace = true) {
        ...
        if (!EntityManager.GetHomeBaseList().Contains(entity)) {
            ...
            // GameObject CreateEffect = Resources.Load(GameConstDefine.LoadGameBuildingEffectPath + jianTaBaoZha) as
            // GameObject; ResourceUnit CreateEffectUnit =
            // ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadGameBuildingEffectPath + jianTaBaoZha,
            // ResourceType.PREFAB); GameObject CreateEffect = CreateEffectUnit.Asset as GameObject;

            NormalEffect normalEffect = EffectManager.Instance.CreateNormalEffect(
                GameConstDefine.LoadGameBuildingEffectPath + jianTaBaoZha, null);
    ...
    public static void DebugWaring(object message) { return; }

    public static void DebugLog(object message) { return; }
    ...
    public static List<string> GetSplit() { return split; }
