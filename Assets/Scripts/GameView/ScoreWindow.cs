...
public class ScoreWindow : BaseWindow {
    ...
    void SetExpLabel(float expIndex, float ExpMax) { mExpNum.text = expIndex + " / " + ExpMax; }

    private void BackRoom(GameObject go) { ScoreCtrl.Instance.AskBackRoom(); }
    ...
    // 显示
    public override void OnEnable() { RefreshData(); }
    ...
    public void RefreshData() {
        ...
        foreach (var item in ScoreCtrl.Instance.GetSettlementList()) {
            if (item.mCamp == EntityCampType.CampTypeA) {
                mListPlayer[i++].SetShowLabel((UInt64)item.mHeadID,
                                              item.mNickName,
                                              item.mLevel,
                                              item.mKills,
                                              item.mDeath,
                ...
                if (num <= 5)
                    mListPlayer[num++].SetShowLabel((UInt64)item.mHeadID,
                                                    item.mNickName,
                                                    item.mLevel,
                                                    item.mKills,
                                                    item.mDeath,
    ...
    private void ShowBattleResult() {
        ...
        if (player.StateSituation == SITUATION.WIN) {
            // BlGame.AudioManager.Instance.PlayLongVoiceAudio(Resources.Load(AudioDefine.PATH_HERO_KILLSOUND +
            // "Victory") as AudioClip);

            ResourceUnit clipUnit = ResourcesManager.Instance.loadImmediate(
                AudioDefine.PATH_HERO_KILLSOUND + "Victory", ResourceType.ASSET);
            ...
            ResourceUnit clipUnit = ResourcesManager.Instance.loadImmediate(
                AudioDefine.PATH_HERO_KILLSOUND + "Failed", ResourceType.ASSET);
            AudioClip clip = clipUnit.Asset as AudioClip;
    ...
    public class UIShowScoreInfo {
        ...
        public void SetShowLabel(
            UInt64 headIcon, string name, int level, int kill, int deathtimes, int asstime, int totalcp, int lasthit) {
