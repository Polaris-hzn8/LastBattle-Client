...
public class SystemNoticeWindow : BaseWindow {
    ...
    private void Close(GameObject go) { SystemNoticeCtrl.Instance.Exit(); }
    ...
    // ——升序/降序\n4.具备预览功能——前端显示匹配\n5.文本具备居中/左对齐/右对齐选择",
    // "特点：登入游戏后弹出，左右两侧分栏显示，左侧为公告标题，右侧为公告正文区域，\n两侧皆可    上下滑动。",
    // "左上角为公告属性，包括“活动”，“促销”，“维护”等字眼；", "右下角为公告状态，包括“NEW（新）”，“HOT（火爆）”字眼" };
    void SetAllNotice() {
        mLastObj = null;
        ...
        foreach (var item in SystemNoticeData.Instance.systemNotList) {
            ResourceUnit objUnit =
                ResourcesManager.Instance.loadImmediate(GameConstDefine.LoadNoticeTitleUI, ResourceType.PREFAB);
    ...
    public override void OnEnable() { SetAllNotice(); }
    // 隐藏
