...
public class Ientity {
    ...
    public EntityCampType EntityCamp { get; set; }
    ...
    public float MpMax { private set; get; }

    public int Level { private set; get; }

    public float Cp { private set; get; }

    public float TeamCp { private set; get; }

    public float Exp { private set; get; }

    public int CooldownReduce { set; get; }

    public XueTiaoUI XueTiao { private set; get; }

    public EntityType entityType { get; set; }

    public bool IfNPC() { return entityType < EntityType.Player; }

    /// <summary>
    /// Npc类型
    /// </summary>
    public int NpcGUIDType { get; set; }

    public string ModelName { get; set; }

    public float ColliderRadius { get; set; }

    public ENPCCateChild NPCCateChild { get; set; }

    public GameObject realObject { set; get; }

    public Entity RealEntity { set; get; }

    public EntityFSM FSM { private set; get; }
...
#region State change
    public Vector3 EntityFSMPosition { set; get; }

    public Vector3 EntityFSMDirection { private set; get; }

    public float EntityFSMMoveSpeed { private set; get; }

    public int EntitySkillID { set; get; }

    public int EntitySkillModelId { private set; get; }

    protected Ientity entitySkillTarget { get; set; }
    ...
    /// 复活
    /// </summary>
    public virtual void OnReborn() { this.DoReborn(); }
    ...
    public void EntityFSMChangeDataOnPrepareSkill(
        Vector3 mvPos, Vector3 mvDir, int skillID, Ientity targetID, int isAbsorbSkill = 0) {
    ...
    /// 强制位移
    /// </summary>
    public void EntityFSMChangeDataOnForceMove(int skId) { EntitySkillModelId = skId; }
    ...
    // 引导技能
    public virtual void OnEntityLeadSkill() { RealEntity.PlayerFreeAnimation(); }
        ...
        if (skillManagerConfig.isAbsorb == 1) {
            ...
            if (skillManagerConfig.isNormalAttack == 1) {
            ...
            } else {
                {
                    ...
                    if (RealEntity.GetComponent<Animation>()[skillManagerConfig.rAnimation] != null &&
                        RealEntity.GetComponent<Animation>()[skillManagerConfig.rAnimation].wrapMode != WrapMode.Loop) {
    ...
    public virtual void OnCreateShadow() {
        ...
        // GameObject obj = UnityEngine.Object.Instantiate(Resources.Load(GameDefine.GameConstDefine.LoadGameShadow)) as
        // GameObject;
        ResourceUnit objUnit =
            ResourcesManager.Instance.loadImmediate(GameDefine.GameConstDefine.LoadGameShadow, ResourceType.PREFAB);
    ...
    private void PlayDeadSound() {
        ...
        if (deadSound != "" && deadSound != "0" && deadSound != null) {
            ...
            ResourceUnit clipUnit = ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_HERO_DEADSOUND + deadSound,
                                                                            ResourceType.ASSET);
    ...
    public virtual void SetMp(float curMp) { Mp = curMp; }

    public virtual void SetHp(float curHp) { Hp = curHp; }

    public virtual void SetHpMax(float curHpMax) { HpMax = curHpMax; }

    public virtual void SetMpMax(float curMpMax) { MpMax = curMpMax; }

    public virtual void SetLevel(int curLv) { Level = curLv; }

    public virtual void SetExp(float curExp) { Exp = curExp; }

    public void SetCp(float curCp) { Cp = curCp; }

    public void SetTeamCp(float curCp) { TeamCp = curCp; }
    ...
    public void OnEntityFingerClick() {
        ...
        if (pl != null) {
            ...
            if (ConfigReader.NpcXmlInfoDict.ContainsKey(NpcGUIDType) &&
                ConfigReader.NpcXmlInfoDict[NpcGUIDType].un32ShopID != 0) {
        ...
        // if (Vector3.Distance(pl.realObject.transform.position, this.realObject.transform.position) >
    ...
    public void OpenShop(int id = 0) {
        if (UIShop.Instance == null) {
            ...
            ResourceUnit clipUnit =
                ResourcesManager.Instance.loadImmediate(AudioDefine.PATH_CLICKSHOP_SOUND, ResourceType.ASSET);
            AudioClip clip = clipUnit.Asset as AudioClip;
    ...
    public void SetJungleMonsterBeAtkVoice() {
        ...
        if (entityType != EntityType.Monster || Hp < hpLimit ||
            AudioManager.Instance.HeroLinesAudioDict.ContainsKey(GameObjGUID))
            return;
    ...
    // 被击飞效果
    public virtual void OnBeatFly(uint buffId,
                                  string action,
                                  float raiseSpeed,
                                  float raiseAccSpeed,
                                  float fallSpeed,
        ...
        if (realObject != null) {
            ...
            beatFlyMotion.BeatFly(
                buffId, action, raiseSpeed, raiseAccSpeed, fallSpeed, fallAccSpeed, stayTime, canRecover);
        }
    }
