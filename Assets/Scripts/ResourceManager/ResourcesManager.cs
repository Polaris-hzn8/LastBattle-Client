...
public class ResourcesManager : UnitySingleton<ResourcesManager> {
        ...
        if (UsedAssetBundle) {
            ...
            switch (mCurrentRequest.mRequestType) {
                    ...
                    switch (mCurrentRequest.mResourceType) {
                        ...
                        case ResourceType.PREFAB: {
                            if (mLoadedResourceUnit.ContainsKey(fileName)) {
                                ...
                                mCurrentRequest.mResourceAsyncOperation.mResource =
                                    mLoadedResourceUnit[fileName] as ResourceUnit;
                                ...
                                // 传入相对路径名称
                                // StartCoroutine(_load(fileName, mCurrentRequest.mHandle,
                                // mCurrentRequest.mResourceType, mCurrentRequest.mResourceAsyncOperation));
                            }
                        } break;
                        case ResourceType.LEVELASSET: {
                            DebugEx.LogError("do you real need a single level asset??? this is have not decide!!!",
                                             ResourceCommon.DEBUGTYPENAME);
                        } break;
                ...
                case RequestType.LOADLEVEL: {
                    StartCoroutine(_loadLevel(fileName,
                                              mCurrentRequest.mHandleLevel,
                                              ResourceType.LEVEL,
                                              mCurrentRequest.mResourceAsyncOperation));
                } break;
            ...
            switch (mCurrentRequest.mRequestType) {
                case RequestType.LOAD: {
                    switch (mCurrentRequest.mResourceType) {
                        ...
                        case ResourceType.PREFAB: {
                            // 暂时不处理，直接使用资源相对路径
                            // StartCoroutine(_load(mCurrentRequest.mFileName, mCurrentRequest.mHandle,
                            // mCurrentRequest.mResourceType, mCurrentRequest.mResourceAsyncOperation));
                        } break;
                        case ResourceType.LEVELASSET: {
                            DebugEx.LogError("do you real need a single level asset??? this is have not decide!!!",
                ...
                case RequestType.LOADLEVEL: {
                    StartCoroutine(_loadLevel(mCurrentRequest.mFileName,
                                              mCurrentRequest.mHandleLevel,
                                              ResourceType.LEVEL,
                                              mCurrentRequest.mResourceAsyncOperation));
                } break;
    ...
    private void handleResponse() { mCurrentRequest = null; }

    // 传入Resources下相对路径名称 例如Resources/Game/Effect1    传入Game/Effect1
    public ResourceUnit loadImmediate(string filePathName,
                                      ResourceType resourceType,
                                      string archiveName = "Resources") {
        // 使用assetbundle打包
        if (UsedAssetBundle) {
            // 添加Resource
        ...
        // 不使用
            ...
            Debug.Log("resource load " + filePathName);
        ...
        //        DebugEx.LogError("the assetbundle create error!!! and the asset is : " + path, "CreateAssetBunlde",
        //        true);
        ...
        //                (loadedAsset.GetType() == typeof(Texture) || loadedAsset.GetType() == typeof(Texture2D) ||
        //                loadedAsset.GetType() == typeof(Texture3D)))
        //            {
        //                Material mat = asset as Material;
        //                if
        //                (!mAssetExtraInfo.mhandleDeleteAssetBundleExtraInfo.mAllTextureNameInShader.ContainsKey(loadedAsset.name))
        //                    DebugEx.LogError("the asset : " + loadedAsset.name + " dependenced by : " + path + " but
        //                    the asset is not exist in mAllTextureNameInShader, so maybe is your material and shader is
        //                    not match!!!", ResourceCommon.DEBUGTYPENAME);
        //                else
        //                    mat.SetTexture(mAssetExtraInfo.mhandleDeleteAssetBundleExtraInfo.mAllTextureNameInShader[loadedAsset.name],
        //                    loadedAsset as Texture);
        //            }
        //        }
        //    }

        //    ResourceUnit resource = new ResourceUnit(mAssetExtraInfo.mCanDeleteAssetbundle ||
        //    (mAssetExtraInfo.mSingleDependenced && mAssetExtraInfo.mNoUpeerDependenced) ? null : assetBundle,
        //    assetBundleSize, asset, path, resourceType, mAssetExtraInfo.mAllDependencesAssetSize); if (null !=
        //    mAssetExtraInfo.mAllDps)
        //    {
        ...
        //    mAssetExtraInfo.mNoUpeerDependenced))
    ...
    // public ResourceAsyncOperation load(string filePathName, HandleFinishLoad handle, ResourceType type, string
    // archiveName = "Resources")
    //{
    //     //使用assetbundle打包
    //     if (UsedAssetBundle)
    //     {
    //         //ResourceCommon.Log("load : " + fileName + " and the type is : " + type.ToString() + " and the
    //         archiveName is : " + archiveName);
    ...
    //        {
    //            ResourceAsyncOperation operation = new ResourceAsyncOperation(RequestType.LOAD);
    //            mAllRequests.Enqueue(new Request(filePathName, archiveName, type, handle, RequestType.LOAD,
    ...
    // fileName = "Scene/1"
    public ResourceAsyncOperation loadLevel(string fileName,
                                            HandleFinishLoadLevel handle,
        ...
        if (UsedAssetBundle) {
            ...
            if (mLoadedResourceUnit.ContainsKey(completeName)) {
                DebugEx.LogError("why you load same level twice, maybe you have not unload last time!!!",
                                 ResourceCommon.DEBUGTYPENAME);
                return null;
            } else {
                ResourceAsyncOperation operation = new ResourceAsyncOperation(RequestType.LOADLEVEL);
                mAllRequests.Enqueue(
                    new Request(completeName, ResourceType.LEVEL, handle, RequestType.LOADLEVEL, operation));
                return operation;
            }
        }
        // 不使用
    ...
    // private IEnumerator _load(string path, HandleFinishLoad handle, ResourceType resourceType, ResourceAsyncOperation
    // operation)
    //{
    //     //使用assetbundle打包
    //     if (UsedAssetBundle)
    ...
    //        yield return StartCoroutine(ResourceCommon._CreateFromMemory(path, delegate(AssetBundleCreateRequest
    //        request, int size)
    //        {
    ...
    //        //if (0 != mAssetExtraInfo.mAllDps.Count)
    //        //    yield return StartCoroutine(loadDependenceAsset(mAssetExtraInfo.mAllDps, ResourceType.ASSET,
    //        operation));

    //        //AssetBundleRequest abr = null;
    //        //yield return StartCoroutine(ResourceCommon.LoadAsyncFromAssetBundle(assetBundle, name, typeof(Object),
    //        delegate(AssetBundleRequest request)
    //        //{
    //        //    abr = request;
    //        //}));
    ...
    //        //                (asset.GetType() == typeof(Texture) || asset.GetType() == typeof(Texture2D) ||
    //        asset.GetType() == typeof(Texture3D)))
    //        //            {
    //        //                Material mat = abr.asset as Material;
    //        //                if
    //        (!mAssetExtraInfo.mhandleDeleteAssetBundleExtraInfo.mAllTextureNameInShader.ContainsKey(asset.name))
    //        //                    DebugEx.LogError("the asset : " + asset.name + " dependenced by : " + path + " but
    //        the asset is not exist in mAllTextureNameInShader, so maybe is your material and shader is not match!!!",
    //        ResourceCommon.DEBUGTYPENAME);
    //        // mat.SetTexture(mAssetExtraInfo.mhandleDeleteAssetBundleExtraInfo.mAllTextureNameInShader[asset.name],
    //        asset as Texture);
    //        //            }
    //        //        }
    //        //    }
    ...
    //        //ResourceUnit resource = new ResourceUnit(mAssetExtraInfo.mCanDeleteAssetbundle ||
    //        (mAssetExtraInfo.mSingleDependenced && mAssetExtraInfo.mNoUpeerDependenced) ? null : assetBundle,
    //        assetBundleSize, abr.asset, path, resourceType, mAssetExtraInfo.mAllDependencesAssetSize);
    ...
    //        //    DebugEx.LogError("mLoadDependencesAssetSize and mAllDependencesAssetSize are not match in asset :" +
    //        path, ResourceCommon.DEBUGTYPENAME);
    //        //operation.mComplete = true;
    ...
    private IEnumerator _loadLevel(string path,
                                   HandleFinishLoadLevel handle,
                                   ResourceType resourceType,
                                   ResourceAsyncOperation operation) {
        // 使用assetbundle打包
        if (UsedAssetBundle) {
            // 根据场景名称获取asset信息
            ...
            // int assetBundleSize = 0;
            // AssetBundleCreateRequest bundleRequest = null;
            // yield return StartCoroutine(ResourceCommon._CreateFromMemory(path + "_depsinfo",
            // delegate(AssetBundleCreateRequest request, int size)
            //{
            //     bundleRequest = request;
            //     assetBundleSize = size;
            // }));
            ...
            // ResourceUnit resource = new ResourceUnit(null, assetBundleSize, null, path, resourceType,
            // operation.mAllDependencesAssetSize);
    ...
    //        AssetBundle.CreateFromMemoryImmediate(ResourceCommon.getAssetBundleFileBytes(path, ref assetBundleSize));
    ...
    //            DebugEx.LogError("mNoUpperDependenced asset apper in loadDependenceAsset is impossible!!!",
    //            ResourceCommon.DEBUGTYPENAME);
    ...
    //                if
    //                (!mAssetExtraInfo.mhandleDeleteAssetBundleExtraInfo.mAllTextureNameInShader.ContainsKey(loadedAsset.name))
    //                    DebugEx.LogError("the asset : " + loadedAsset.name + " dependenced by : " + path + " but the
    //                    asset is not exist in mAllTextureNameInShader, so maybe is your material and shader is not
    //                    match!!!", ResourceCommon.DEBUGTYPENAME);
    //                else
    //                    mat.SetTexture(mAssetExtraInfo.mhandleDeleteAssetBundleExtraInfo.mAllTextureNameInShader[loadedAsset.name],
    //                    loadedAsset as Texture);
    //            }
    ...
    //        ResourceUnit ru = new ResourceUnit((mAssetExtraInfo.mCanDeleteAssetbundle ||
    //        mAssetExtraInfo.mSingleDependenced) ? null : assetBundle, assetBundleSize, asset, path, type,
    //        mAssetExtraInfo.mAllDependencesAssetSize); if (null != mAssetExtraInfo.mAllDps)
    //        {
    //            foreach (string p in mAssetExtraInfo.mAllDps)
    //            {
    //                string depcompleteName = ResourceCommon.getFileName(p, true);
    ...
    // private IEnumerator loadDependenceAsset(List<string> dependences, ResourceType type, ResourceAsyncOperation
    // operation)
    //{
    //     foreach (string path in dependences)
    //     {
    ...
    //         if (mLoadedResourceUnit.ContainsKey(completeName))
    //         {
    //             operation.mLoadDependencesAssetSize += (mLoadedResourceUnit[completeName] as
    //             ResourceUnit).AllDependencesAssetSize; continue;
    //         }
    ...
    //        AssetBundleRequest abr = null;
    //        yield return StartCoroutine(ResourceCommon.LoadAsyncFromAssetBundle(assetBundle, completeName,
    //        typeof(Object), delegate(AssetBundleRequest request)
    //        {
    //            abr = request;
    //        }));
    ...
    //                ////处理材质
    //                if (abr.asset.GetType() == typeof(Material) &&
    //                    (asset.GetType() == typeof(Texture) || asset.GetType() == typeof(Texture2D) || asset.GetType()
    //                    == typeof(Texture3D)))
    //                {
    //                    Material mat = abr.asset as Material;
    //                    if
    //                    (!mAssetExtraInfo.mhandleDeleteAssetBundleExtraInfo.mAllTextureNameInShader.ContainsKey(asset.name))
    //                        DebugEx.LogError("this is impossible,the deps in runtiume not match when build. the asset
    //                        is :" + path, ResourceCommon.DEBUGTYPENAME);
    //                    mat.SetTexture(mAssetExtraInfo.mhandleDeleteAssetBundleExtraInfo.mAllTextureNameInShader[asset.name],
    //                    asset as Texture);
    //                }
    //            }
    ...
    //        ResourceUnit ru = new ResourceUnit((mAssetExtraInfo.mCanDeleteAssetbundle ||
    //        mAssetExtraInfo.mSingleDependenced) ? null : assetBundle, assetBundleSize, abr.asset, path, type,
    //        mAssetExtraInfo.mAllDependencesAssetSize); if (null != mAssetExtraInfo.mAllDps)
    //        {
    //            foreach (string p in mAssetExtraInfo.mAllDps)
    //            {
    ...
    //     ResourceCommon.assetbundleFileSuffix); if (!File.Exists(localPath))//原始包中
    //     {
    ...
    public static Stream Open(string path) {
        ...
        if (Application.platform == RuntimePlatform.Android || Application.platform == RuntimePlatform.IPhonePlayer) {
            localPath =
                string.Format("{0}/{1}", Application.persistentDataPath, path + ResourceCommon.assetbundleFileSuffix);
        }
    ...
    //     }
    //     else if (Application.platform == RuntimePlatform.WindowsEditor || Application.platform ==
