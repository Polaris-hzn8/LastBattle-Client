...
public class GuideFlashTask : GuideTaskBase {
    ...
    public GuideFlashTask(int task, GuideTaskType type, GameObject mParent) : base(task, type, mParent) {
        // 读取数据
        ...
        if (!string.IsNullOrEmpty(flashTask.UiPath)) {
            ...
            ResourceUnit objFlashUnit =
                ResourcesManager.Instance.loadImmediate(flashTask.PrefabPath, ResourceType.PREFAB);
    ...
    public override void EnterTask() {
        switch (flashTask.Type) {
            ...
            case FlashType.FlashMove:
                tween = TweenPosition.Begin(
                    objFlash, flashTask.During, GameMethod.ResolveToVector3(flashTask.TargetEffect, ';'));
                break;
            case FlashType.FlashScale:
                tween = TweenScale.Begin(
                    objFlash, flashTask.During, GameMethod.ResolveToVector3(flashTask.TargetEffect, ';'));
                break;
        }
